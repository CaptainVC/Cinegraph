# Private speaker-review third-adjudication submission

Phase 73 exposes `speaker-review-submit-third-adjudication-v1`, a one-shot paid
submission of exactly Terra adjudication part three. It is intentionally a
...
Phase 72 checkpoint with exactly two completed parts.
...
- The review-host bootstrap check succeeds after installing the tenth forced
  command. Refresh the reviewed host files after deploying this release.
- The complete preparation-through-Phase-72 root intent/receipt chain is
  present, root-owned, and unmodified.
- The selected run is `adjudication_part_completed`, has
  `adjudication_completed_part_count == 2`, has more than two total
  adjudication parts, and contains exactly two unique batch/input ID pairs.
- Completed request, journal, and output evidence for parts one and two is
  present. Part-three output and API-error artifacts must not exist.
...
After the Phase 73 release is active, run the reviewed bootstrap using the
...
```json
{"archive_sha256":"<64 lowercase hex>","authorization_id":"<uuid-v4>","maximum_authorized_cost_microusd":5000000,"operation":"submit_third_adjudication","purpose":"speaker_review","run_id":"speaker-review-<16 lowercase hex>","schema_version":1,"season_number":2}
```
...
```zsh
uv run python scripts/private_speaker_review_third_adjudication_submission_client.py \
...
inventory digest classes, the part-three request digest, deterministic Terra
...
two and the target to part three. The only permitted durable changes are:

- create the part-three submission-intent journal;
- create the matching part-three submission-completed journal;
- append one unique batch/input ID pair; and
- move to `adjudication_submitted` while keeping completed count at two.
...
- `submitted`: part three was submitted, or an exact completed application
...
still `2`, total count is greater than `2`, ID arrays contain exactly three
unique entries, the part-three journal pair exists, and part-three output does
...
authorization. Part three must be observed by a future distinct command and
authorization; this Phase 73 boundary is the required stop point.
