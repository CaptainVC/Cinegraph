...
# Root-owned, no-argument wrapper for one bounded single v1 next-primary transition.
set -euo pipefail
...
readonly CONTAINER_NAME="cinegraph-speaker-review-submit-next-primary"
readonly COMPOSE_SERVICE="corpus-speaker-review-submit-next-primary"

fail() {
    printf '%s\n' "private next-primary review transition rejected" >&2
    exit 1
}
...
cleanup_worker() {
    ...
    expected_image="$(
        timeout --signal=TERM --kill-after="${KILL_AFTER_SECONDS}s" "${KILL_AFTER_SECONDS}s" \
            ...
            --env-file "$ENV_FILE" \
            --profile corpus-speaker-review-submit-next-primary \
    ...
    [[ "${lines[9]}" == '["python","scripts/submit_next_private_speaker_review_workspace.py"]' ]] || return 0
...
processor="$release_dir/scripts/run_private_speaker_review_next_primary.py"
worker="$release_dir/scripts/submit_next_private_speaker_review_workspace.py"
next_contract="$release_dir/scripts/private_speaker_review_next_primary_submission_contract.py"
next_host_contract="$release_dir/scripts/private_speaker_review_next_primary_host_contract.py"
