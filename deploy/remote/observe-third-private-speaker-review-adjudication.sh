...
# Root-owned, no-argument wrapper for one bounded third-adjudication observation.
set -euo pipefail
...
readonly THIRD_ADJUDICATION_SUBMISSION_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/third-adjudication-submission-receipts"
readonly OBSERVATION_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/third-adjudication-observation-receipts"
...
readonly CONTAINER_NAME="cinegraph-speaker-review-observe-third-adjudication"
readonly COMPOSE_SERVICE="corpus-speaker-review-observe-third-adjudication"

fail() {
    printf '%s\n' "private third-adjudication observation rejected" >&2
    exit 1
}
...
cleanup_worker() {
    ...
    expected_image="$(
        timeout --signal=TERM --kill-after="${KILL_AFTER_SECONDS}s" "${KILL_AFTER_SECONDS}s" \
            ...
            --env-file "$ENV_FILE" \
            --profile corpus-speaker-review-observe-third-adjudication \
    ...
    [[ "${lines[9]}" == '["python","scripts/observe_third_private_speaker_review_adjudication_workspace.py"]' ]] || return 0
...
check_root_path "$THIRD_ADJUDICATION_SUBMISSION_RECEIPTS_ROOT" directory 700
...
processor="$release_dir/scripts/run_private_speaker_review_third_adjudication_observation.py"
worker="$release_dir/scripts/observe_third_private_speaker_review_adjudication_workspace.py"
observation_contract="$release_dir/scripts/private_speaker_review_third_adjudication_observation_contract.py"
observation_host_contract="$release_dir/scripts/private_speaker_review_third_adjudication_observation_host_contract.py"
...
for tracked_name in \
    ...
    scripts/observe_third_private_speaker_review_adjudication_workspace.py \
