...
# Root-owned, no-argument wrapper for the paid primary speaker-review submit.
set -euo pipefail
...
readonly CONTAINER_NAME="cinegraph-speaker-review-submit-primary"

fail() {
    printf '%s\n' "private speaker-review submission rejected" >&2
    exit 1
}
...
processor="$release_dir/scripts/run_private_speaker_review_submission.py"
worker="$release_dir/scripts/submit_private_speaker_review_workspace.py"
submission_contract="$release_dir/scripts/private_speaker_review_submission_contract.py"
host_contract="$release_dir/scripts/private_speaker_review_submission_host_contract.py"
...
if [[ "$status" -ne 0 ]]; then
    docker compose --progress quiet \
        --env-file "$ENV_FILE" \
        --profile corpus-speaker-review-submit-primary \
        -f "$compose" \
        rm --force --stop corpus-speaker-review-submit-primary \
        >/dev/null 2>&1 || true
