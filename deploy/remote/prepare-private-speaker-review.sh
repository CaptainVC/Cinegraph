...
# Root-owned, no-argument wrapper for the offline speaker-review preparation job.
# This boundary deliberately has no provider credential or network access.  The
# later submission phase is a separate, explicitly authorized operation.
set -euo pipefail
...
readonly SPEAKER_REVIEW_SOURCE_ROOT="$SPEAKER_REVIEW_ROOT/source"
readonly SPEAKER_REVIEW_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/receipts"
readonly SPEAKER_REVIEW_RUNS_ROOT="$DEV_CORPUS_ROOT/review-runs"
...
readonly SPEAKER_REVIEW_CONTAINER_NAME="cinegraph-speaker-review-prepare"

fail() {
    printf '%s\n' "private speaker-review preparation rejected the request" >&2
    exit 1
}
...
check_root_path "$SPEAKER_REVIEW_SOURCE_ROOT" directory 700
check_root_path "$SPEAKER_REVIEW_RECEIPTS_ROOT" directory 700
check_root_path "$SPEAKER_REVIEW_RUNS_ROOT" directory 700
check_root_path "$ENV_FILE" file 600
check_root_path /usr/local/sbin/cinegraph-prepare-private-speaker-review file 755

umask 077
# Lock order is invariant across transfer, deploy, and all private workers.
...
processor="$release_dir/scripts/run_private_speaker_review.py"
worker="$release_dir/scripts/prepare_private_speaker_review_workspace.py"
review_contract="$release_dir/scripts/private_speaker_review_contract.py"
...
shared_runtime="$release_dir/scripts/run_private_corpus_processing.py"
compose="$release_dir/deploy/compose.yaml"
for tracked_file in "$processor" "$worker" "$review_contract" "$host_contract" "$processing_contract" "$receiver" "$shared_runtime" "$compose"; do
...
for tracked_name in \
    ...
    scripts/prepare_private_speaker_review_workspace.py \
...
# GNU timeout creates a dedicated process group by default (deliberately do not
# use --foreground), so a deadline reaches the Python coordinator and all of
# its descendants.  Compose cleanup below handles a daemon-side one-off
# container if the client is interrupted before `--rm` can remove it.
set +e
env -i PATH=/usr/sbin:/usr/bin SUDO_USER=cinegraph-corpus \
...
if [[ "$status" -ne 0 ]]; then
    docker compose --progress quiet \
        --env-file "$ENV_FILE" \
        --profile corpus-speaker-review \
        -f "$compose" \
        rm --force --stop corpus-speaker-review-prepare \
        >/dev/null 2>&1 || true
    docker rm --force "$SPEAKER_REVIEW_CONTAINER_NAME" >/dev/null 2>&1 || true
fi
exit "$status"
