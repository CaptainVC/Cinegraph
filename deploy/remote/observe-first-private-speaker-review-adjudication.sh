...
# Root-owned, no-argument wrapper for one bounded first-adjudication observation.
set -euo pipefail
...
readonly FIRST_ADJUDICATION_SUBMISSION_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/first-adjudication-submission-receipts"
readonly OBSERVATION_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/first-adjudication-observation-receipts"
...
readonly CONTAINER_NAME="cinegraph-speaker-review-observe-first-adjudication"
readonly COMPOSE_SERVICE="corpus-speaker-review-observe-first-adjudication"

fail() {
    printf '%s\n' "private first-adjudication observation rejected" >&2
    exit 1
}
...
cleanup_worker() {
    ...
    expected_image="$(
        timeout --signal=TERM --kill-after="${KILL_AFTER_SECONDS}s" "${KILL_AFTER_SECONDS}s" \
            ...
            --env-file "$ENV_FILE" \
            --profile corpus-speaker-review-observe-first-adjudication \
    ...
    [[ "${lines[9]}" == '["python","scripts/observe_first_private_speaker_review_adjudication_workspace.py"]' ]] || return 0
...
check_root_path "$FIRST_ADJUDICATION_SUBMISSION_RECEIPTS_ROOT" directory 700
...
processor="$release_dir/scripts/run_private_speaker_review_first_adjudication_observation.py"
worker="$release_dir/scripts/observe_first_private_speaker_review_adjudication_workspace.py"
observation_contract="$release_dir/scripts/private_speaker_review_first_adjudication_observation_contract.py"
observation_host_contract="$release_dir/scripts/private_speaker_review_first_adjudication_observation_host_contract.py"
