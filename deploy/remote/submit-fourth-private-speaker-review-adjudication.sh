...
# Root-owned, no-argument wrapper for one bounded fourth-adjudication submission.
set -euo pipefail
...
readonly NEXT_ADJUDICATION_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/next-adjudication-submission-receipts"
readonly NEXT_ADJUDICATION_OBSERVATION_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/next-adjudication-observation-receipts"
readonly THIRD_ADJUDICATION_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/third-adjudication-submission-receipts"
readonly THIRD_ADJUDICATION_OBSERVATION_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/third-adjudication-observation-receipts"
readonly FOURTH_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/fourth-adjudication-submission-receipts"
...
readonly CONTAINER_NAME="cinegraph-speaker-review-submit-fourth-adjudication"
readonly COMPOSE_SERVICE="corpus-speaker-review-submit-fourth-adjudication"

fail() {
    printf '%s\n' "private fourth-adjudication submission rejected" >&2
    exit 1
}
...
cleanup_worker() {
    ...
    expected_image="$({
        timeout --signal=TERM --kill-after="${KILL_AFTER_SECONDS}s" "${KILL_AFTER_SECONDS}s" \
            ...
            --profile corpus-speaker-review-submit-fourth-adjudication \
    ...
    [[ "${lines[9]}" == '["python","scripts/submit_fourth_private_speaker_review_adjudication_workspace.py"]' ]] || return 0
    ...
    for mount in "${lines[@]:17}"; do
        case "$mount" in
            *"|/review-workspace/review-runs/"*"|true")
                ...
                [[ "$review_mount_source" =~ ^/opt/cinegraph/shared/private-corpus/dev/review-runs/sha256-[0-9a-f]{64}/review-runs/(speaker-review-[0-9a-f]{16})$ ]] || return 0
                [[ "$mount" == "$review_mount_source|/review-workspace/review-runs/${BASH_REMATCH[1]}|true" ]] || return 0
                review_mount="$mount"
                ;;
...
check_root_path "$NEXT_ADJUDICATION_RECEIPTS_ROOT" directory 700
check_root_path "$NEXT_ADJUDICATION_OBSERVATION_RECEIPTS_ROOT" directory 700
check_root_path "$THIRD_ADJUDICATION_RECEIPTS_ROOT" directory 700
check_root_path "$THIRD_ADJUDICATION_OBSERVATION_RECEIPTS_ROOT" directory 700
check_root_path "$FOURTH_RECEIPTS_ROOT" directory 700
...
processor="$release_dir/scripts/run_private_speaker_review_fourth_adjudication.py"
worker="$release_dir/scripts/submit_fourth_private_speaker_review_adjudication_workspace.py"
fourth_contract="$release_dir/scripts/private_speaker_review_fourth_adjudication_submission_contract.py"
fourth_host_contract="$release_dir/scripts/private_speaker_review_fourth_adjudication_host_contract.py"
compose="$release_dir/deploy/compose.yaml"
trusted_files=(
    "$processor"
    "$worker"
    "$fourth_contract"
    "$fourth_host_contract"
    ...
    "$release_dir/src/cinegraph/__init__.py"
    "$release_dir/src/cinegraph/common/private_corpus_bundle.py"
    "$release_dir/src/cinegraph/common/private_corpus_policy.py"
...
for tracked_name in \
    ...
    src/cinegraph/__init__.py \
    src/cinegraph/common/private_corpus_bundle.py \
    src/cinegraph/common/private_corpus_policy.py \
