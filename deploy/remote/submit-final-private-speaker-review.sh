...
fail() { printf '%s\n' 'private final-review submission rejected' >&2; exit 1; }
check_root_path() {
...
readonly RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/final-review-submission-receipts"
...
# Every directory and tracked file can influence a stdlib-only transitive
# import. Validate the complete clean release while the deployment lock is
# held, rather than trusting only the explicit critical-file list below.
directory_count=0
...
processor="$release_dir/scripts/run_private_speaker_review_final_review.py"
worker="$release_dir/scripts/submit_final_private_speaker_review_workspace.py"
contract_file="$release_dir/scripts/private_speaker_review_final_review_submission_contract.py"
host_contract="$release_dir/scripts/private_speaker_review_final_review_host_contract.py"
compose="$release_dir/deploy/compose.yaml"
for tracked_file in "$processor" "$worker" "$contract_file" "$host_contract" "$compose" "$release_dir/src/cinegraph/ingestion/speaker_review/workflow.py" "$release_dir/src/cinegraph/adapters/workflow/langgraph/speaker_review_graph.py" "$release_dir/src/cinegraph/common/speaker_review_cost_policy.py" "$release_dir/scripts/run_private_speaker_review_adjudication_result_processing.py" "$release_dir/scripts/private_speaker_review_adjudication_result_processing_contract.py"; do
