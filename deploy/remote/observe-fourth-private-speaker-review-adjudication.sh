...
# Root-owned, no-argument wrapper for one bounded fourth-adjudication observation.
set -euo pipefail
...
readonly FOURTH_ADJUDICATION_SUBMISSION_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/fourth-adjudication-submission-receipts"
readonly OBSERVATION_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/fourth-adjudication-observation-receipts"
...
readonly CONTAINER_NAME="cinegraph-speaker-review-observe-fourth-adjudication"
readonly COMPOSE_SERVICE="corpus-speaker-review-observe-fourth-adjudication"

fail() {
    printf '%s\n' "private fourth-adjudication observation rejected" >&2
    exit 1
}
...
cleanup_worker() {
    ...
    expected_image="$(
        timeout --signal=TERM --kill-after="${KILL_AFTER_SECONDS}s" "${KILL_AFTER_SECONDS}s" \
            ...
            --env-file "$ENV_FILE" \
            --profile corpus-speaker-review-observe-fourth-adjudication \
    ...
    [[ "${lines[9]}" == '["python","scripts/observe_fourth_private_speaker_review_adjudication_workspace.py"]' ]] || return 0
...
check_root_path "$FOURTH_ADJUDICATION_SUBMISSION_RECEIPTS_ROOT" directory 700
...
processor="$release_dir/scripts/run_private_speaker_review_fourth_adjudication_observation.py"
worker="$release_dir/scripts/observe_fourth_private_speaker_review_adjudication_workspace.py"
observation_contract="$release_dir/scripts/private_speaker_review_fourth_adjudication_observation_contract.py"
observation_host_contract="$release_dir/scripts/private_speaker_review_fourth_adjudication_observation_host_contract.py"
