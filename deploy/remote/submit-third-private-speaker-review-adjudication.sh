...
# Root-owned, no-argument wrapper for one bounded third-adjudication submission.
set -euo pipefail
...
readonly THIRD_RECEIPTS_ROOT="$SPEAKER_REVIEW_ROOT/third-adjudication-submission-receipts"
...
readonly CONTAINER_NAME="cinegraph-speaker-review-submit-third-adjudication"
readonly COMPOSE_SERVICE="corpus-speaker-review-submit-third-adjudication"

fail() {
    printf '%s\n' "private third-adjudication submission rejected" >&2
    exit 1
}
...
cleanup_worker() {
    ...
    expected_image="$({
        timeout --signal=TERM --kill-after="${KILL_AFTER_SECONDS}s" "${KILL_AFTER_SECONDS}s" \
            ...
            --profile corpus-speaker-review-submit-third-adjudication \
    ...
    [[ "${lines[9]}" == '["python","scripts/submit_third_private_speaker_review_adjudication_workspace.py"]' ]] || return 0
...
check_root_path "$THIRD_RECEIPTS_ROOT" directory 700
...
processor="$release_dir/scripts/run_private_speaker_review_third_adjudication.py"
worker="$release_dir/scripts/submit_third_private_speaker_review_adjudication_workspace.py"
third_contract="$release_dir/scripts/private_speaker_review_third_adjudication_submission_contract.py"
third_host_contract="$release_dir/scripts/private_speaker_review_third_adjudication_host_contract.py"
