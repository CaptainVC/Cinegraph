...
def _submit(
    ...
    return client.submit_fourth_adjudication(
        archive_sha256=DIGEST,
        run_id=RUN_ID,
        authorization_id=AUTHORIZATION_ID,
        maximum_authorized_cost_microusd=5_000_000,
...
def test_client_rejects_unbound_non_part_three_or_over_budget_response(
    ...
    with pytest.raises(client.FourthAdjudicationSubmissionClientError):
...
def test_client_rejects_host_command_drift(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(host, "REVIEW_FOURTH_ADJUDICATION_COMMAND", "unexpected")
    with pytest.raises(client.FourthAdjudicationSubmissionClientError):
