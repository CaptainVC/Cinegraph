...
def test_third_submission_isolated_inventory_policy_and_total_cost() -> None:
...
state = {{
    ...
    'adjudication_part_count': 3, 'adjudication_completed_part_count': 2,
    'adjudication_batch_id': 'batch-2', 'adjudication_input_file_id': 'file-2',
    'adjudication_batch_ids': ['batch-1', 'batch-2'],
    'adjudication_input_file_ids': ['file-1', 'file-2'],
}}
...
assert root._estimate_cost(contents, view) == 11545
assert 'openai' not in sys.modules and 'qdrant_client' not in sys.modules
print('isolated-third-policy-ok')
"""
    ...
    assert result.stdout.strip() == "isolated-third-policy-ok"


@pytest.mark.skipif(os.name != "posix", reason="Host inventory enforces POSIX ownership and modes")
def test_isolated_third_submission_reads_real_inventory_and_reaches_worker(
    tmp_path: Path,
) -> None:
...
for part, batch, input_file in ((1, "batch-1", "file-1"), (2, "batch-2", "file-2")):
...
root._validate_phase72_predecessor = lambda *args: (prep, "b" * 64, "c" * 64, "d" * 64, 11545, 100000)
writes = []
...
assert "openai" not in sys.modules and "qdrant_client" not in sys.modules
print("isolated-third-process-ok")
'''
    result = subprocess.run(
        [sys.executable, "-I", "-S", "-B", "-c", code],
        cwd=project,
    ...
    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == "isolated-third-process-ok"
