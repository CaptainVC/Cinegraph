...
import pytest
from scripts import private_speaker_review_next_primary_submission_client as client
...
def _submit(
    ...
    monkeypatch.setattr(client.base.shutil, "which", lambda _: "ssh")
    monkeypatch.setattr(client.base, "_run_ssh", run)
    return client.submit_next_primary(
        archive_sha256=DIGEST,
        run_id=RUN_ID,
        authorization_id=AUTHORIZATION_ID,
        maximum_authorized_cost_microusd=5_000_000,
...
def test_client_sends_only_canonical_request_over_pinned_command(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    result = _submit(tmp_path, monkeypatch, run)
...
def test_client_rejects_invalid_or_private_remote_output(
    ...
    with pytest.raises(client.NextPrimarySubmissionClientError):
        _submit(tmp_path, monkeypatch, run)
...
def test_cli_returns_only_generic_error(
    ...
    monkeypatch.setattr(
        ...
        lambda **_: (_ for _ in ()).throw(
            client.NextPrimarySubmissionClientError("sk-private /private/path batch-private")
        ),
    )
    ...
    assert status == 2
    assert captured.out == ""
    assert captured.err == ('{"error":"speaker_review_next_primary_rejected","status":"error"}\n')
