...
import pytest
from scripts import private_speaker_review_fourth_adjudication_observation_client as client
...
def _aggregate(status: str = "observed") -> dict[str, object]:
    completed = 4 if status in {"observed", "already_observed"} else 3
    return {
        ...
        if status in {"observed", "already_observed"}
        else "failed"
        if status == "failed"
...
def _observe(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, runner: object) -> dict[str, object]:
    ...
    return client.observe_fourth_adjudication(
        archive_sha256=DIGEST,
        run_id=RUN_ID,
        authorization_id=AUTHORIZATION_ID,
        maximum_authorized_cost_microusd=5_000_000,
...
def test_client_sends_canonical_request_over_pinned_fourth_command(
    ...
    assert all(secret not in str(captured) for secret in ("provider_batch_id", "OPENAI_API_KEY"))


@pytest.mark.parametrize("status", ["waiting", "observed", "already_observed", "failed", "reconciliation_required"])
def test_client_accepts_every_contract_status(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, status: str
) -> None:
    result = _aggregate(status)
    monkeypatch.setattr(
        client.base,
        "_run_ssh",
        lambda arguments, _: subprocess.CompletedProcess(arguments, 0, contract.canonical_json(result), b""),
    )
...
def test_client_rejects_malformed_oversized_or_private_output(
    tmp_path: Path,
    ...
    with pytest.raises(client.FourthAdjudicationObservationClientError):
        _observe(tmp_path, monkeypatch, runner)


def test_contract_requires_phase75_count_and_exact_fourth_transition() -> None:
    request = _request()
    assert contract.parse_request(contract.canonical_json(request)) == request
    for change in (
        {"operation": "observe_third_adjudication"},
        {"adjudication_completed_part_count": 3, "status": "observed"},
        {"adjudication_completed_part_count": 4, "status": "waiting"},
...
def test_contract_rejects_duplicate_wire_keys() -> None:
    duplicate = (
        b'{"archive_sha256":"' + DIGEST.encode() + b'","archive_sha256":"' + DIGEST.encode()
        + b'","authorization_id":"' + AUTHORIZATION_ID.encode()
        + b'","maximum_authorized_cost_microusd":5000000,"operation":"observe_fourth_adjudication",'
        + b'"purpose":"speaker_review","run_id":"' + RUN_ID.encode()
...
def test_cli_error_is_generic_and_path_free(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    monkeypatch.setattr(
        client,
        "observe_fourth_adjudication",
        lambda **_: (_ for _ in ()).throw(client.FourthAdjudicationObservationClientError("sk-private /secret")),
    )
    assert client.main(
        [
    ...
    assert captured.out == ""
    assert captured.err == '{"error":"speaker_review_fourth_adjudication_observation_rejected","status":"error"}\n'
