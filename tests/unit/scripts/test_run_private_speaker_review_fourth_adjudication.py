...
def _files(state: SpeakerReviewRunState) -> dict[str, bytes]:
    value = {
        ...
        **{
        ...
        "adjudication-part-0003-output.jsonl": b"output-3\n",
    }
    if state.status is SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED:
        value[root._INTENT_NAME.format(part=4)] = b"intent-4\n"
        value[root._COMPLETED_NAME.format(part=4)] = b"completed-4\n"
    return value
...
def test_host_boundary_is_limited_to_phase74_authenticated_part_four() -> None:
    root._validate_state_shape(_state())
    with pytest.raises(root.FourthAdjudicationSubmissionError, match="checkpoint|predecessor"):
...
def test_worker_arguments_bind_exact_run_and_all_six_digests(tmp_path: Path) -> None:
    ...
    assert arguments[-1] == root.host.REVIEW_FOURTH_ADJUDICATION_COMPOSE_SERVICE
    run_id = str(REQUEST["run_id"])
    assert (
        f"{(tmp_path / 'review-runs' / run_id).as_posix()}:"
        f"{(root.host.REVIEW_FOURTH_ADJUDICATION_RUNS_TARGET / run_id).as_posix()}:rw" in arguments
    )
...
def test_root_inventory_rejects_empty_derived_directory(
    ...
    with pytest.raises(root.FourthAdjudicationSubmissionError, match="inventory"):
        root._inventory(run)
...
def test_fresh_process_submits_once_and_writes_intent_then_receipt(
    ...
    assert writes[0][1]["phase74_observation_receipt_sha256"] == "4" * 64
    assert writes[0][1]["fourth_adjudication_part_number"] == 4


def test_submitted_checkpoint_validates_phase74_against_reconstructed_pre_state(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(root, "_validate_phase74_predecessor", predecessor)
    ...
    assert len(predecessor_states[0].adjudication_batch_ids) == 3
...
def test_replay_rejects_stale_root_intent_before_worker(
    ...
    with pytest.raises(root.FourthAdjudicationSubmissionError, match="binding changed"):
...
def test_worker_result_contract_and_post_inventory_are_fail_closed() -> None:
    ...
    with pytest.raises(root.FourthAdjudicationSubmissionError, match="worker result"):
...
def test_phase74_predecessor_reconstructs_exact_pre_observation_checkpoint(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    state = _state()
    ...
    intent = {key: "fixture" for key in root.phase74._BINDING_KEYS}
    intent.update(
        {
            "authorization_id": AUTHORIZATION_ID,
            "authorization_sha256": root._sha(b"phase74-auth"),
            "estimated_adjudication_cost_microusd": 200_000,
            "maximum_authorized_cost_microusd": 5_000_000,
            "pre_updated_at": "2026-01-01T00:00:00+00:00",
        }
    )
    receipt = {key: "fixture" for key in root.phase74._RECEIPT_KEYS}
    ...
    monkeypatch.setattr(root, "PHASE74_RECEIPTS_ROOT", tmp_path)
    ...
    monkeypatch.setattr(
        root,
        "_phase74_request",
        lambda *_: (phase_request, b"phase74-auth", root._sha(b"phase74-auth")),
    )
    monkeypatch.setattr(root.phase74, "_validate_intent", lambda value, **_: value)
    ...
    monkeypatch.setattr(root.phase74, "_validate_submission_predecessor", validate_submission)
    monkeypatch.setattr(
        root.phase74_contract,
        "validate_aggregate",
    ...
    monkeypatch.setattr(root.phase74, "_validate_final_receipt", validate_final)
    monkeypatch.setattr(
        root.preparation,
    ...
    result = root._validate_phase74_predecessor(REQUEST, tmp_path, contents, state)
    ...
    assert pre_state.adjudication_completed_part_count == 2
    assert len(pre_state.adjudication_batch_ids) == 3
    pre_contents = observed["pre_contents"]
    assert isinstance(pre_contents, dict)
    assert "adjudication-part-0003-output.jsonl" not in pre_contents
    post_contents = observed["post_contents"]
    assert isinstance(post_contents, dict)
    assert "adjudication-part-0003-output.jsonl" in post_contents
    assert root._INTENT_NAME.format(part=4) not in post_contents
    assert result[2:4] == ("1" * 64, "2" * 64)


def test_root_intent_is_pinned_to_part_four() -> None:
    state = _state()
    binding = root._binding(
        ...
        phase74_intent_sha256="5" * 64,
        phase74_receipt_sha256="6" * 64,
        estimated=200_000,
        contents=_files(state),
        state=state,
    )
    assert binding["adjudication_completed_part_count"] == 3
    assert binding["fourth_adjudication_part_number"] == 4
    root._validate_binding(binding, REQUEST)
    with pytest.raises(root.FourthAdjudicationSubmissionError):
        root._validate_binding({**binding, "fourth_adjudication_part_number": 5}, REQUEST)


def test_three_part_checkpoint_is_provider_free_aggregate_only(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    state = replace(_state(), adjudication_part_count=3)
    contents = _files(state)
    artifacts, journals, outputs, derived = root._classes(contents)
    monkeypatch.setattr(root, "RECEIPTS_ROOT", tmp_path)
    monkeypatch.setattr(root, "_validate_authorization", lambda _: "a" * 64)
    monkeypatch.setattr(root, "_run_directory", lambda _: tmp_path)
    monkeypatch.setattr(
        root,
        "_inventory",
        lambda *_: (contents, artifacts, journals, outputs, derived, state),
    )
    monkeypatch.setattr(root, "_validate_completed_evidence", lambda *_: None)
    monkeypatch.setattr(
        root,
        "_validate_phase74_predecessor",
        lambda *_: ({"config_sha": "c" * 64, "image": "i", "release_sha": "r" * 40}, "b" * 64, "c" * 64, "d" * 64, 200_000, 100_000),
    )
    monkeypatch.setattr(root, "_run_worker", lambda *_: pytest.fail("worker must not run"))
    monkeypatch.setattr(root, "_write_once", lambda *_: pytest.fail("receipt must not be written"))
    monkeypatch.setattr(
        root.preparation,
        "_active_binding",
        lambda: ("r" * 40, "i", "c" * 64),
    ...
    assert result["estimated_adjudication_cost_microusd"] == 0
...
@pytest.mark.parametrize(
    ...
    [
        f"{RUN_ID}.intent.json",
        f"{RUN_ID}.json",
        f".{RUN_ID}.intent.json.pending",
        f".{RUN_ID}.json.pending",
    ],
)
def test_three_part_checkpoint_rejects_phase75_root_evidence(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    filename: str,
) -> None:
    ...
    receipts = tmp_path / "receipts"
    receipts.mkdir()
    (receipts / filename).write_bytes(b"unexpected\n")
    monkeypatch.setattr(root, "RECEIPTS_ROOT", receipts)
    monkeypatch.setattr(root, "_validate_authorization", lambda _: "a" * 64)
    monkeypatch.setattr(root, "_run_directory", lambda _: tmp_path)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        "_validate_phase74_predecessor",
        lambda *_: (
            {"config_sha": "c" * 64, "image": "i", "release_sha": "r" * 40},
            "b" * 64,
            "c" * 64,
            "d" * 64,
            200_000,
...
def test_three_part_checkpoint_rejects_part_four_application_journal(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    state = replace(_state(), adjudication_part_count=3)
    contents = _files(state)
    contents[root._INTENT_NAME.format(part=4)] = b"unexpected\n"
    artifacts, journals, outputs, derived = root._classes(contents)
    monkeypatch.setattr(root, "RECEIPTS_ROOT", tmp_path)
    monkeypatch.setattr(root, "_validate_authorization", lambda _: "a" * 64)
    monkeypatch.setattr(root, "_run_directory", lambda _: tmp_path)
    ...
    with pytest.raises(root.FourthAdjudicationSubmissionError, match="ambiguous"):
        root.process_request(REQUEST)
