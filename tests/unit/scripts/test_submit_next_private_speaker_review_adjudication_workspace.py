...
from scripts import submit_next_private_speaker_review_adjudication_workspace as worker
...
def _state(
    ...
    return SpeakerReviewRunState(
        ...
        adjudication_batch_id="batch-2" if submitted else "batch-1",
        adjudication_input_file_id="file-2" if submitted else "file-1",
        adjudication_batch_ids=("batch-1", "batch-2") if submitted else ("batch-1",),
        adjudication_input_file_ids=("file-1", "file-2") if submitted else ("file-1",),
    )
...
def test_fresh_submission_reads_secret_only_after_preflight(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    state = _state()
    ...
    result = worker.submit_next_adjudication(
...
@pytest.mark.parametrize(
    "journal_names",
    [
        {".adjudication-part-0002-submission-intent.json"},
        {".adjudication-part-0002-submission-completed.json"},
    ],
)
def test_partial_application_journal_requires_reconciliation_without_secret(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
...
def test_matching_completed_journal_recovers_without_secret_or_provider(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    state = _state()
    contents = {
        "run-state.json": b"state\n",
        "adjudication-part-0002-requests.jsonl": b'{"body":{}}\n',
        ".adjudication-part-0002-submission-intent.json": b"intent\n",
        ".adjudication-part-0002-submission-completed.json": b"completed\n",
    }
    _patch_preflight(monkeypatch, tmp_path, state, contents)
    monkeypatch.setattr(
        worker,
...
def test_active_output_is_rejected_before_secret_access(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    active_name: str,
) -> None:
    ...
    with pytest.raises(worker.NextAdjudicationSubmissionWorkerError):
        worker.submit_next_adjudication(
            environment=_environment(), review_root=tmp_path
        )
...
def test_digest_mismatch_stops_before_secret_access(
    ...
    state = _state()
    monkeypatch.setattr(worker, "_run_directory", lambda *_: tmp_path)
    monkeypatch.setattr(worker, "_inventory", lambda *_: ({"run-state.json": b"state"}, state))
    monkeypatch.setattr(
        worker,
        "_digest_bindings",
        lambda *_: (_ for _ in ()).throw(
            worker.NextAdjudicationSubmissionWorkerError("changed")
        ),
    )
    monkeypatch.setattr(
        worker,
    ...
    with pytest.raises(worker.NextAdjudicationSubmissionWorkerError, match="changed"):
        worker.submit_next_adjudication(
            environment=_environment(), review_root=tmp_path
        )


def test_contract_rejects_success_with_completed_run_status() -> None:
    aggregate = worker._aggregate(
        _state(SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED),
        status="submitted",
        estimated=100,
        submitted=1,
    )
    aggregate["run_status"] = "adjudication_part_completed"
