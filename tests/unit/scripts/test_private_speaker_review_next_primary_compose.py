...
def test_next_primary_worker_has_dedicated_egress_only_profile() -> None:
    ...
    assert "profiles: [corpus-speaker-review-submit-next-primary]" in service
    assert "- egress" in service
    assert "- backend" not in service
    assert "network_mode: none" not in service
    assert (
        'command: ["python", "scripts/submit_next_private_speaker_review_workspace.py"]'
        in service
    )


def test_next_primary_worker_excludes_sources_and_private_services() -> None:
    service = _service()
...
def test_next_primary_worker_is_unprivileged_and_resource_bounded() -> None:
    service = _service()
