...
import pytest
from scripts import private_speaker_review_first_adjudication_observation_client as client
...
def _observe(
    ...
    return client.observe_first_adjudication(
        archive_sha256=DIGEST,
        run_id=RUN_ID,
        authorization_id=AUTHORIZATION_ID,
        maximum_authorized_cost_microusd=5_000_000,
...
def test_client_rejects_invalid_or_private_remote_output(
    ...
    with pytest.raises(client.FirstAdjudicationObservationClientError):
...
def test_cli_returns_only_generic_error(
    ...
    monkeypatch.setattr(
        client,
        "observe_first_adjudication",
        lambda **_: (_ for _ in ()).throw(
            client.FirstAdjudicationObservationClientError("sk-private /private/path batch-private")
        ),
    )
    ...
    assert captured.out == ""
    assert captured.err == (
        '{"error":"speaker_review_first_adjudication_observation_rejected","status":"error"}\n'
    )
