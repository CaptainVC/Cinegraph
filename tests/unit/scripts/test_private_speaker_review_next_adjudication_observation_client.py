...
import pytest
from scripts import private_speaker_review_next_adjudication_observation_client as client
...
def _aggregate(*, status: str = "observed") -> dict[str, object]:
    completed = 2 if status in {"observed", "already_observed"} else 1
    run_status = (
...
def _observe(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, runner: object) -> dict[str, object]:
    ...
    return client.observe_next_adjudication(
        archive_sha256=DIGEST,
        run_id=RUN_ID,
        authorization_id=AUTHORIZATION_ID,
        maximum_authorized_cost_microusd=5_000_000,
...
def test_client_rejects_malformed_private_or_oversized_remote_output(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, stdout: bytes, stderr: bytes, returncode: int) -> None:
    ...
    with pytest.raises(client.NextAdjudicationObservationClientError):
...
def test_contract_rejects_wrong_status_count_id_or_duplicate_wire_keys() -> None:
    ...
    with pytest.raises(ValueError):
        contract.validate_request({**request, "operation": "submit_next_adjudication"})
    for change in (
        {"adjudication_completed_part_count": 1, "status": "observed"},
    ...
    duplicate = b'{"archive_sha256":"' + DIGEST.encode() + b'","archive_sha256":"' + DIGEST.encode() + b'","authorization_id":"' + AUTHORIZATION_ID.encode() + b'","maximum_authorized_cost_microusd":5000000,"operation":"observe_next_adjudication","purpose":"speaker_review","run_id":"' + RUN_ID.encode() + b'","schema_version":1,"season_number":2}\n'
    with pytest.raises(ValueError):
...
def test_cli_never_leaks_private_error_details(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    monkeypatch.setattr(client, "observe_next_adjudication", lambda **_: (_ for _ in ()).throw(client.NextAdjudicationObservationClientError("sk-private /secret")))
    ...
    assert captured.out == ""
    assert captured.err == '{"error":"speaker_review_next_adjudication_observation_rejected","status":"error"}\n'
