...
import pytest
from scripts import run_private_speaker_review_next_adjudication_observation as root
...
def _model(status: SpeakerReviewRunStatus = SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED, completed: int = 1) -> SpeakerReviewRunState:
...
def _install(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, before: SpeakerReviewRunState, after: SpeakerReviewRunState, *, worker_status: str) -> tuple[list[tuple[Path, dict[str, object]]], Path]:
    run = tmp_path / RUN_ID
    run.mkdir()
    receipts = tmp_path / "next-adjudication-observation-receipts"
    receipts.mkdir()
    after_snapshot = (_contents(after, observed=worker_status == "observed"), _payload(after), after)
    ...
    monkeypatch.setattr(root, "_validate_submission_predecessor", lambda *_: ({"config_sha": "c" * 64, "image": "image@sha256:" + "9" * 64, "release_sha": "8" * 40}, "b" * 64, "c" * 64, "d" * 64, 500_000))
    ...
    monkeypatch.setattr(root.phase70.submit.phase68, "_source_workspace", lambda *_: None)
    ...
    estimate = 500_000
    monkeypatch.setattr(root, "_run_worker", lambda *_: root._aggregate(REQUEST, after.to_dict(), worker_status, estimate))
    return writes, run


def test_part_two_success_writes_exact_intent_and_completion_receipt(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    writes, _ = _install(tmp_path, monkeypatch, _model(), _model(SpeakerReviewRunStatus.ADJUDICATION_PART_COMPLETED, 2), worker_status="observed")
...
def test_waiting_has_no_after_state_mutation_and_no_completion_receipt(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    before = _model()
...
def test_missing_root_receipt_requires_reconciliation_without_provider_access(
    ...
    observed = _model(SpeakerReviewRunStatus.ADJUDICATION_PART_COMPLETED, 2)
    run = tmp_path / RUN_ID
    run.mkdir()
    receipts = tmp_path / "next-adjudication-observation-receipts"
    receipts.mkdir()
    ...
    monkeypatch.setattr(root, "_validate_authorization", lambda _: "a" * 64)
    monkeypatch.setattr(root, "_run_directory", lambda _: run)
    monkeypatch.setattr(root, "_inventory", lambda *_: (_contents(observed, observed=True), _payload(observed), observed))
    ...
    with pytest.raises(
        root.NextAdjudicationObservationError,
...
def test_root_boundary_is_part_two_only_and_rejects_wrong_count_or_duplicate_ids() -> None:
    valid = _payload(_model())
    root._validate_submitted_state(valid)
    for invalid in (
        _payload(replace(_model(), adjudication_completed_part_count=2)),
        ...
        with pytest.raises(root.NextAdjudicationObservationError):
            root._validate_submitted_state(invalid)


def test_inventory_binding_requires_the_exact_run_state_schema(monkeypatch: pytest.MonkeyPatch) -> None:
    model = _model()
    run = Path(RUN_ID)
    payload = model.to_dict()
    monkeypatch.setattr(
        root.phase70.submit,
        "_inventory",
        lambda *_: ({"run-state.json": b"state"}, {}, {}, {}, {"state": payload, "derived": ()}),
    )
    monkeypatch.setattr(root.submit, "load_validated_run_state", lambda *_: (run, model))
    monkeypatch.setattr(root.worker, "_expected_inventory_names", lambda *_args, **_kwargs: ({"run-state.json"}, set()), raising=False)
    contents, state, returned = root._inventory(run)
    assert contents == {"run-state.json": b"state"}
    ...
    monkeypatch.setattr(
        root.phase70.submit,
        "_inventory",
        lambda *_: ({"run-state.json": b"state"}, {}, {}, {}, {"state": {"run_id": RUN_ID}, "derived": ()}),
    )
    monkeypatch.setattr(root.submit, "load_validated_run_state", lambda *_: (run, Incomplete()))
    with pytest.raises(root.NextAdjudicationObservationError, match="inventory"):
        root._inventory(Path("run"))


def test_active_runtime_drift_is_rejected_after_worker(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _install(tmp_path, monkeypatch, _model(), _model(), worker_status="waiting")
    monkeypatch.setattr(root.preparation, "_active_binding", lambda: ("drift" * 8, "other", "different"))
    with pytest.raises(root.NextAdjudicationObservationError, match="runtime"):
        root.process_request(REQUEST)
...
def test_worker_args_carry_request_and_all_six_verified_checkpoint_bindings(tmp_path: Path) -> None:
    ...
    assert args[-1] == root.host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_COMPOSE_SERVICE


def test_main_returns_generic_error_without_private_details(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    monkeypatch.setattr(root, "_require_root", lambda: None)
    monkeypatch.setattr(root, "_read_request", lambda *_: (_ for _ in ()).throw(RuntimeError("batch-private /secret/path")))
    assert root.main() == 2
    captured = capsys.readouterr()
    assert captured.out == ""
    assert captured.err == "error=speaker_review_next_adjudication_observation_rejected\n"
