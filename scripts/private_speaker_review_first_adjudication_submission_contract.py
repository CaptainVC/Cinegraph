"""Strict wire contract for the first private speaker-review adjudication submit."""
...
COMMAND: Final = "speaker-review-submit-first-adjudication-v1"
PROTOCOL_VERSION: Final = 1
PURPOSE: Final = "speaker_review"
SEASON_NUMBER: Final = 2
OPERATION: Final = "submit_first_adjudication"
...
RUN_STATUSES: Final = frozenset({"adjudication_prepared", "adjudication_submitted"})
...
def _decode(raw: bytes, *, maximum: int) -> dict[str, object]:
    if (
        ...
        raise ValueError("invalid first-adjudication wire value")
    try:
        value = json.loads(
            raw.decode("utf-8"),
        ...
        raise ValueError("invalid first-adjudication wire value") from error
...
def validate_request(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        raise ValueError("invalid first-adjudication request")
...
def validate_aggregate(value: object, *, status: str | None = None) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != AGGREGATE_KEYS:
        raise ValueError("invalid first-adjudication aggregate")
    ...
    if (
        type(value.get("adjudication_part_count")) is not int
        or value["adjudication_part_count"] <= 0
    ):
        raise ValueError("invalid first-adjudication aggregate")
    if type(value.get("submitted_part_count")) is not int or value["submitted_part_count"] not in (
        0,
        1,
    ):
        raise ValueError("invalid first-adjudication aggregate")
    if expected in {"submitted", "already_submitted"} and (
        value["run_status"] != "adjudication_submitted" or value["submitted_part_count"] != 1
    ):
        raise ValueError("invalid first-adjudication aggregate")
    if expected == "reconciliation_required" and value["submitted_part_count"] != 0:
        raise ValueError("invalid first-adjudication aggregate")
    if expected == "reconciliation_required" and value["run_status"] != "adjudication_prepared":
        raise ValueError("invalid first-adjudication aggregate")
    return dict(value)
...
def is_reconciliation_error(error: BaseException) -> bool:
    from cinegraph.common.error_messages import SpeakerReviewErrorMessages  # noqa: I001
