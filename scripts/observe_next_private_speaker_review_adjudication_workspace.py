"""Observe exactly the next adjudication Batch for a private review run.
...
from scripts import (
    private_speaker_review_next_adjudication_observation_contract as contract,  # noqa: E402
)
...
class NextAdjudicationObservationWorkerError(RuntimeError):
...
def read_stable_openai_secret(path: Path = OPENAI_SECRET_PATH) -> str:
    ...
    try:
        ...
        raise NextAdjudicationObservationWorkerError("observation secret unavailable") from error
    finally:
        if descriptor >= 0:
            os.close(descriptor)
    if (
        ...
        raise NextAdjudicationObservationWorkerError("observation secret unavailable")
    try:
...
def request_from_environment(environment: Mapping[str, str] | None = None) -> dict[str, object]:
    ...
    try:
        ...
        raise NextAdjudicationObservationWorkerError("observation request invalid") from error
...
def _parse_cost_cap(value: str) -> int:
    try:
        ...
        raise NextAdjudicationObservationWorkerError("observation request invalid") from error
    if str(parsed) != value:
        raise NextAdjudicationObservationWorkerError("observation request invalid")
...
def _cost_microusd(value: float) -> int:
    if not isinstance(value, (int, float)) or isinstance(value, bool) or not math.isfinite(value):
        raise NextAdjudicationObservationWorkerError("observation cost invalid")
    try:
        micros = Decimal(str(value)) * Decimal(1_000_000)
    except (InvalidOperation, ValueError) as error:
        raise NextAdjudicationObservationWorkerError("observation cost invalid") from error
...
def _run_directory(run_id: str, review_root: Path = PRIVATE_REVIEW_RUNS_ROOT) -> Path:
    ...
    try:
        if candidate.resolve(strict=False).parent != root:
            raise NextAdjudicationObservationWorkerError("observation run unavailable")
    except OSError as error:
        raise NextAdjudicationObservationWorkerError("observation run unavailable") from error
    return candidate
...
def _read_checkpoint_file(path: Path, maximum: int) -> bytes:
    ...
    try:
        ...
        raise NextAdjudicationObservationWorkerError(
    ...
    if (
        ...
        raise NextAdjudicationObservationWorkerError("observation checkpoint changed")
    return raw


def _expected_hash(value: str, label: str) -> str:
    if len(value) != 64 or any(character not in "0123456789abcdef" for character in value):
        raise NextAdjudicationObservationWorkerError(f"observation {label} invalid")
    return value
...
def _validate_expected_checkpoint(
    ...
    if any(value is None for value in expected.values()):
        raise NextAdjudicationObservationWorkerError("observation checkpoint invalid")
    if (
        ...
        or len(state.adjudication_batch_ids) != state.adjudication_completed_part_count + 1
        or len(state.adjudication_input_file_ids) != state.adjudication_completed_part_count + 1
    ...
    for name in sorted(names):
        ...
        if total > 256 * 1024 * 1024:
            raise NextAdjudicationObservationWorkerError("observation checkpoint unavailable")
        contents[name] = raw
...
def _aggregate(
    ...
    *,
    ...
    try:
        ...
        raise NextAdjudicationObservationWorkerError("observation result invalid") from error
...
def _validate_state_transition(
    ...
    if (
        ...
        raise NextAdjudicationObservationWorkerError("observation result invalid")
...
def observe_next_adjudication(
    *,
    ...
    if state.status is SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED:
        ...
        raise NextAdjudicationObservationWorkerError("observation run state invalid")
    ...
    if expected_total > cap or expected_total > configuration_cap:
        raise NextAdjudicationObservationWorkerError("observation cost exceeds authorization")
    # A completed next adjudication part is the only safe replay state for this
    ...
    if state.status is SpeakerReviewRunStatus.ADJUDICATION_PART_COMPLETED:
        ...
        try:
            ...
            raise NextAdjudicationObservationWorkerError(
                "observation replay invalid"
            ) from error
        if returned_directory != canonical or replayed != state:
            raise NextAdjudicationObservationWorkerError("observation replay invalid")
        return _aggregate(
            state,
    ...
    try:
        ...
        raise NextAdjudicationObservationWorkerError(
            "next adjudication observation failed"
        ) from error
...
def main() -> int:
    try:
        result = observe_next_adjudication()
        sys.stdout.buffer.write(contract.canonical_json(result))
        return 0
    except NextAdjudicationObservationWorkerError:
        sys.stderr.write("error=speaker_review_next_adjudication_observation_failed\n")
        return 2
    except Exception:
        sys.stderr.write("error=speaker_review_next_adjudication_observation_failed\n")
        return 2
