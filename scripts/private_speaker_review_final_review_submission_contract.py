"""Strict wire contract for bounded final-review part-one submission."""
...
RUN_STATUSES: Final = frozenset({"final_review_prepared", "final_review_submitted"})
...
# Compatibility aliases used by provider-free processing boundaries.  The
# PRE_* spellings are canonical for this command; aliases keep the digest
# names centralized when callers share generic inventory code.
ENV_EXPECTED_STATE_DIGEST: Final = ENV_EXPECTED_PRE_RUN_STATE_SHA256
ENV_EXPECTED_ARTIFACTS_DIGEST: Final = ENV_EXPECTED_PRE_ARTIFACT_SET_SHA256
ENV_EXPECTED_JOURNALS_DIGEST: Final = ENV_EXPECTED_PRE_JOURNAL_SET_SHA256
ENV_EXPECTED_OUTPUTS_DIGEST: Final = ENV_EXPECTED_PRE_OUTPUT_SET_SHA256
ENV_EXPECTED_DERIVED_DIGEST: Final = ENV_EXPECTED_PRE_DERIVED_SET_SHA256
...
def _decode(raw: bytes, *, maximum: int) -> dict[str, object]:
    if (
        ...
        raise ValueError("invalid final-review wire value")
    try:
        value = json.loads(
            raw.decode("utf-8"),
        ...
        raise ValueError("invalid final-review wire value") from error
...
def validate_request(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        raise ValueError("invalid final-review request")
...
def validate_aggregate(value: object, *, status: str | None = None) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != AGGREGATE_KEYS:
        raise ValueError("invalid final-review aggregate")
    ...
    _cost(value.get("actual_adjudication_cost_microusd"), positive=False)
    _cost(value.get("estimated_final_review_cost_microusd"), positive=False)
    part_count = value.get("final_review_part_count")
    completed_count = value.get("final_review_completed_part_count")
    ...
    if (
        ...
        raise ValueError("invalid final-review aggregate")
    if expected in {"submitted", "already_submitted"} and (
        submitted_count != 1 or value["run_status"] != "final_review_submitted"
    ):
        raise ValueError("invalid final-review aggregate")
    if expected == "reconciliation_required" and (
        submitted_count != 0 or value["run_status"] != "final_review_prepared"
    ):
        raise ValueError("invalid final-review aggregate")
    return dict(value)
