"""Submit exactly one subsequent Terra adjudication part.

This egress worker receives only a digest-bound ``review-runs`` parent and an
OpenAI Compose secret.  It submits part ``k + 1`` from an
``adjudication_part_completed`` checkpoint and never observes, parses, advances,
finalizes, or ingests a review.
"""
...
from scripts import private_speaker_review_next_adjudication_submission_contract as contract  # noqa: E402
...
class NextAdjudicationSubmissionWorkerError(RuntimeError):
...
def _stable(path: Path, maximum: int) -> bytes:
    ...
    try:
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication evidence unavailable") from error
    finally:
        if descriptor >= 0:
            os.close(descriptor)
...
def read_stable_openai_secret(path: Path = OPENAI_SECRET_PATH) -> str:
    ...
    try:
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication secret unavailable") from error
    try:
    ...
    if not secret or any(character.isspace() for character in secret):
        raise NextAdjudicationSubmissionWorkerError("adjudication secret unavailable")
    return secret
...
def _parse_cost_cap(value: str) -> int:
    try:
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication request invalid") from error
    if str(parsed) != value or parsed <= 0 or parsed > contract.MAXIMUM_AUTHORIZED_COST_MICROUSD:
        raise NextAdjudicationSubmissionWorkerError("adjudication request invalid")
    return parsed
...
def _run_directory(run_id: str, review_root: Path = PRIVATE_REVIEW_RUNS_ROOT) -> Path:
    try:
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication run unavailable") from error
...
def _inventory(run: Path) -> tuple[dict[str, bytes], SpeakerReviewRunState]:
    try:
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication inventory unavailable") from error
    if STATE not in names:
        raise NextAdjudicationSubmissionWorkerError("adjudication inventory invalid")
    try:
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication run state invalid") from error
    ...
    for name in sorted(names):
        ...
        if total > TOTAL_MAX_BYTES:
            raise NextAdjudicationSubmissionWorkerError("adjudication inventory too large")
        contents[name] = raw
    try:
    ...
    if state.status not in {
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication checkpoint invalid")
...
def _digest_bindings(contents: Mapping[str, bytes], environment: Mapping[str, str]) -> None:
    ...
    if any(
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication checkpoint binding invalid")
    ...
    if actual != expected:
        raise NextAdjudicationSubmissionWorkerError("adjudication checkpoint changed")
...
def _expected_request_hash(
    ...
    if not isinstance(expected, str) or len(expected) != 64 or any(
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication request binding invalid")
    ...
    if name not in contents or _sha(contents[name]) != expected:
        raise NextAdjudicationSubmissionWorkerError("adjudication request changed")
    return expected
...
def _parse_requests(
    ...
    for part in range(1, state.adjudication_part_count + 1):
        ...
        if raw is None:
            raise NextAdjudicationSubmissionWorkerError("adjudication request unavailable")
        try:
...
def _cost_micros(value: float) -> int:
    if not isinstance(value, (int, float)) or isinstance(value, bool) or not math.isfinite(value):
        raise NextAdjudicationSubmissionWorkerError("adjudication cost invalid")
    try:
        micros = Decimal(str(value)) * Decimal(1_000_000)
    except (InvalidOperation, ValueError) as error:
        raise NextAdjudicationSubmissionWorkerError("adjudication cost invalid") from error
    if micros < 0:
        raise NextAdjudicationSubmissionWorkerError("adjudication cost invalid")
...
def _aggregate(
    ...
    *,
    ...
    try:
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication aggregate invalid") from error
...
def _validate_journal(
    ...
    *,
    ...
    try:
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication evidence invalid") from error
    ...
    if (
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication evidence invalid")
...
def _validate_completed_parts(
    ...
    for part in range(1, count + 1):
        ...
        if f"adjudication-part-{part:04d}-output.jsonl" not in contents:
            raise NextAdjudicationSubmissionWorkerError("adjudication output unavailable")
...
def _validate_replay_evidence(
    ...
    if any(
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication replay output invalid")
...
def submit_next_adjudication(
    *,
    ...
    if state.adjudication_part_count <= 1:
        raise NextAdjudicationSubmissionWorkerError("no adjudication part remains")
    ...
    if actual_primary + estimate_micros > cap or actual_primary + estimate_micros > configured:
        raise NextAdjudicationSubmissionWorkerError("adjudication cost exceeds authorization")
    ...
    if state.status is SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED:
        ...
        try:
            ...
            raise NextAdjudicationSubmissionWorkerError("adjudication replay invalid") from error
        if replay_directory != run or replay_state != state:
            raise NextAdjudicationSubmissionWorkerError("adjudication replay invalid")
        return _aggregate(
            state,
    ...
    try:
        ...
        raise NextAdjudicationSubmissionWorkerError("adjudication submission failed") from error
    if returned != run or updated.status is not SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED:
        raise NextAdjudicationSubmissionWorkerError("adjudication transition invalid")
...
def main() -> int:
    try:
        sys.stdout.buffer.write(contract.canonical_json(submit_next_adjudication()))
        return 0
    except Exception:
        sys.stderr.write("error=speaker_review_next_adjudication_submission_failed\n")
        return 2
