"""Strict wire contract for the separately authorized third-part observer."""
...
COMMAND: Final = "speaker-review-observe-third-adjudication-v1"
PROTOCOL_VERSION: Final = 1
PURPOSE: Final = "speaker_review"
SEASON_NUMBER: Final = 2
OPERATION: Final = "observe_third_adjudication"
MAXIMUM_AUTHORIZED_COST_MICROUSD: Final = 5_000_000
PREDECESSOR_COMPLETED_PART_COUNT: Final = 2
...
def _decode(raw: bytes, *, maximum: int) -> dict[str, object]:
    if (
        ...
        raise ValueError("invalid third-adjudication observation wire value")
    try:
        value = json.loads(
            raw.decode("utf-8"),
        ...
        raise ValueError("invalid third-adjudication observation wire value") from error
...
def validate_request(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        raise ValueError("invalid third-adjudication observation request")
...
def validate_aggregate(value: object, *, status: str | None = None) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != AGGREGATE_KEYS:
        raise ValueError("invalid third-adjudication observation aggregate")
    ...
    # This command is deliberately pinned to the Phase 73 checkpoint.  Waiting
    # and terminal failure retain the two completed parts; a successful
    # observation advances exactly one part to three.
