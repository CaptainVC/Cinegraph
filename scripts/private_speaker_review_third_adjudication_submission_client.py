"""Pinned-SSH client for exactly adjudication part-three submission."""
...
class ThirdAdjudicationSubmissionClientError(RuntimeError):
...
def _contract() -> ModuleType:
    try:
        module = import_module(
            "scripts.private_speaker_review_third_adjudication_submission_contract"
        )
        for name in (
            "COMMAND",
            "OPERATION",
        ...
        raise ThirdAdjudicationSubmissionClientError("submission contract unavailable") from error


def ssh_arguments(*, ssh: str, identity: Path, known_hosts: Path, host: str) -> list[str]:
    policy = import_module("scripts.private_speaker_review_third_adjudication_host_contract")
    if policy.REVIEW_THIRD_ADJUDICATION_COMMAND != _contract().COMMAND:
        raise ThirdAdjudicationSubmissionClientError("submission host policy unavailable")
...
def _run_ssh(arguments: list[str], wire: Path) -> subprocess.CompletedProcess[bytes]:
    policy = import_module("scripts.private_speaker_review_third_adjudication_host_contract")
    timeout = (
        int(policy.REVIEW_THIRD_ADJUDICATION_TIMEOUT_SECONDS)
        + int(policy.REVIEW_THIRD_ADJUDICATION_KILL_AFTER_SECONDS)
        ...
        with wire.open("rb") as source:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                ...
                try:
                    ...
                    process.kill()
                    process.wait()
                    raise ThirdAdjudicationSubmissionClientError("SSH submission failed") from error
                stdout = out.result(timeout=5)
                stderr = err.result(timeout=5)
        return subprocess.CompletedProcess(arguments, code, stdout, stderr)
    except ThirdAdjudicationSubmissionClientError:
        raise
...
def submit_third_adjudication(
    *,
    ...
    try:
        ...
        if ssh is None:
            raise OSError
        with tempfile.TemporaryDirectory(prefix="cinegraph-speaker-third-adjudication-") as name:
        ...
        raise ThirdAdjudicationSubmissionClientError(
            "remote third-adjudication submission failed"
        ) from error
    ...
    if (
        ...
        raise ThirdAdjudicationSubmissionClientError(
            "remote third-adjudication submission rejected"
        )
    try:
        ...
        raise ThirdAdjudicationSubmissionClientError(
            "remote third-adjudication response invalid"
        ) from error
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = submit_third_adjudication(
    ...
    except (ThirdAdjudicationSubmissionClientError, ValueError):
        sys.stderr.write(
            '{"error":"speaker_review_third_adjudication_rejected","status":"error"}\n'
        )
        return 2
