"""Pinned-SSH client for observing exactly adjudication part four."""
...
from scripts import (
    private_speaker_review_fourth_adjudication_observation_host_contract as host_policy,
)
from scripts import private_speaker_review_observation_client as base
from scripts.dev_host_contract import validate_host


class FourthAdjudicationObservationClientError(RuntimeError):
...
def ssh_arguments(*, ssh: str, identity: Path, known_hosts: Path, host: str) -> list[str]:
    ...
    if host_policy.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_COMMAND != contract.COMMAND:
        raise FourthAdjudicationObservationClientError(
            "fourth-adjudication observation host policy unavailable"
        )
    try:
...
def observe_fourth_adjudication(
    *,
    ...
    try:
        ...
        raise FourthAdjudicationObservationClientError("host input is invalid") from error
    try:
        base._regular(identity, private=True)
        base._validate_known_hosts(known_hosts, canonical_host)
    except base.SpeakerReviewObservationClientError as error:
        raise FourthAdjudicationObservationClientError(
            "fourth-adjudication observation input is invalid"
        ) from error
    ssh = shutil.which("ssh")
    if ssh is None:
        raise FourthAdjudicationObservationClientError("OpenSSH client is unavailable")
    try:
        with tempfile.TemporaryDirectory(
            prefix="cinegraph-speaker-observe-fourth-adjudication-"
        ) as directory:
            temp = Path(directory)
            if os.name != "nt":
                temp.chmod(0o700)
            ...
            try:
                ...
                raise FourthAdjudicationObservationClientError(
                    "SSH fourth-adjudication observation failed"
                ) from error
    except FourthAdjudicationObservationClientError:
        raise
    ...
    if (
        ...
        raise FourthAdjudicationObservationClientError(
            "remote fourth-adjudication observation was rejected"
        )
    try:
        ...
        raise FourthAdjudicationObservationClientError(
            "remote fourth-adjudication observation response is invalid"
        ) from error


observe_fourth_adjudication_part = observe_fourth_adjudication
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (FourthAdjudicationObservationClientError, ValueError):
        sys.stderr.buffer.write(
            b'{"error":"speaker_review_fourth_adjudication_observation_rejected","status":"error"}\n'
        )
        return 2
