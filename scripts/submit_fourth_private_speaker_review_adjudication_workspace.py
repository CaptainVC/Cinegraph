"""Submit exactly Terra adjudication part four.
...
the independently pinned completed-count-three checkpoint and never observes,
...
from scripts import private_speaker_review_fourth_adjudication_submission_contract as contract  # noqa: E402
...
class FourthAdjudicationSubmissionWorkerError(RuntimeError):
...
def _stable(path: Path, maximum: int) -> bytes:
    ...
    try:
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication evidence unavailable") from error
    finally:
        if descriptor >= 0:
            os.close(descriptor)
...
def read_stable_openai_secret(path: Path = OPENAI_SECRET_PATH) -> str:
    ...
    try:
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication secret unavailable") from error
    try:
    ...
    if not secret or any(character.isspace() for character in secret):
        raise FourthAdjudicationSubmissionWorkerError("adjudication secret unavailable")
    return secret
...
def _parse_cost_cap(value: str) -> int:
    try:
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication request invalid") from error
    if str(parsed) != value or parsed <= 0 or parsed > contract.MAXIMUM_AUTHORIZED_COST_MICROUSD:
        raise FourthAdjudicationSubmissionWorkerError("adjudication request invalid")
    return parsed
...
def _run_directory(run_id: str, review_root: Path = PRIVATE_REVIEW_RUNS_ROOT) -> Path:
    try:
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication run unavailable") from error
...
def _inventory(run: Path) -> tuple[dict[str, bytes], SpeakerReviewRunState]:
    try:
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication inventory unavailable") from error
    if STATE not in names:
        raise FourthAdjudicationSubmissionWorkerError("adjudication inventory invalid")
    try:
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication run state invalid") from error
    ...
    for name in sorted(names):
        ...
        if total > TOTAL_MAX_BYTES:
            raise FourthAdjudicationSubmissionWorkerError("adjudication inventory too large")
        contents[name] = raw
    try:
    ...
    if state.status not in {
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication checkpoint invalid")
...
def _digest_bindings(contents: Mapping[str, bytes], environment: Mapping[str, str]) -> None:
    ...
    if any(
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication checkpoint binding invalid")
    ...
    if actual != expected:
        raise FourthAdjudicationSubmissionWorkerError("adjudication checkpoint changed")
...
def _expected_request_hash(
    ...
    if not isinstance(expected, str) or len(expected) != 64 or any(
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication request binding invalid")
    ...
    if name not in contents or _sha(contents[name]) != expected:
        raise FourthAdjudicationSubmissionWorkerError("adjudication request changed")
    return expected
...
def _parse_requests(
    ...
    for part in range(1, state.adjudication_part_count + 1):
        ...
        if raw is None:
            raise FourthAdjudicationSubmissionWorkerError("adjudication request unavailable")
        try:
...
def _cost_micros(value: float) -> int:
    if not isinstance(value, (int, float)) or isinstance(value, bool) or not math.isfinite(value):
        raise FourthAdjudicationSubmissionWorkerError("adjudication cost invalid")
    try:
        micros = Decimal(str(value)) * Decimal(1_000_000)
    except (InvalidOperation, ValueError) as error:
        raise FourthAdjudicationSubmissionWorkerError("adjudication cost invalid") from error
    if micros < 0:
        raise FourthAdjudicationSubmissionWorkerError("adjudication cost invalid")
...
def _aggregate(
    ...
    *,
    ...
    try:
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication aggregate invalid") from error
...
def _validate_journal(
    ...
    *,
    ...
    try:
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication evidence invalid") from error
    ...
    if (
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication evidence invalid")
...
def _validate_completed_parts(
    ...
    for part in range(1, count + 1):
        ...
        if f"adjudication-part-{part:04d}-output.jsonl" not in contents:
            raise FourthAdjudicationSubmissionWorkerError("adjudication output unavailable")
...
def _validate_checkpoint_shape(state: SpeakerReviewRunState, *, submitted: bool) -> None:
    ...
    if (
        ...
        or count != contract.PREDECESSOR_COMPLETED_PART_COUNT
        or (count >= total and not (count == total == contract.PREDECESSOR_COMPLETED_PART_COUNT and not submitted))
...
def _validate_replay_evidence(
    ...
    if any(
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication replay output invalid")
...
def submit_fourth_adjudication(
    *,
    ...
    if state.adjudication_completed_part_count != contract.PREDECESSOR_COMPLETED_PART_COUNT:
        raise FourthAdjudicationSubmissionWorkerError("adjudication predecessor invalid")
    if (
        ...
        if any(
            ...
            raise FourthAdjudicationSubmissionWorkerError(
                "adjudication checkpoint ambiguous"
            )
        return _aggregate(
            state,
    ...
    if state.adjudication_part_count <= contract.PREDECESSOR_COMPLETED_PART_COUNT:
        raise FourthAdjudicationSubmissionWorkerError("no adjudication part remains")
    ...
    if actual_primary + estimate_micros > cap or actual_primary + estimate_micros > configured:
        raise FourthAdjudicationSubmissionWorkerError("adjudication cost exceeds authorization")
    ...
    if state.status is SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED:
        ...
        try:
            ...
            raise FourthAdjudicationSubmissionWorkerError("adjudication replay invalid") from error
        if replay_directory != run or replay_state != state:
            raise FourthAdjudicationSubmissionWorkerError("adjudication replay invalid")
        return _aggregate(
            state,
    ...
    try:
        ...
        raise FourthAdjudicationSubmissionWorkerError("adjudication submission failed") from error
    if returned != run or updated.status is not SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED:
        raise FourthAdjudicationSubmissionWorkerError("adjudication transition invalid")
...
submit_fourth_adjudication_part = submit_fourth_adjudication


def main() -> int:
    try:
        sys.stdout.buffer.write(contract.canonical_json(submit_fourth_adjudication()))
        return 0
    except Exception:
        sys.stderr.write("error=speaker_review_fourth_adjudication_submission_failed\n")
        return 2
