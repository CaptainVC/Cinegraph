"""Pinned-SSH client for observing exactly adjudication part one."""
...
from scripts import (
    private_speaker_review_first_adjudication_observation_host_contract as host_policy,
)
from scripts import private_speaker_review_observation_client as base
from scripts.dev_host_contract import validate_host


class FirstAdjudicationObservationClientError(RuntimeError):
...
def ssh_arguments(*, ssh: str, identity: Path, known_hosts: Path, host: str) -> list[str]:
    ...
    if host_policy.REVIEW_FIRST_ADJUDICATION_OBSERVATION_COMMAND != contract.COMMAND:
        raise FirstAdjudicationObservationClientError(
            "first-adjudication observation host policy unavailable"
        )
    try:
...
def observe_first_adjudication(
    *,
    ...
    try:
        ...
        raise FirstAdjudicationObservationClientError("host input is invalid") from error
    try:
        base._regular(identity, private=True)
        base._validate_known_hosts(known_hosts, canonical_host)
    except base.SpeakerReviewObservationClientError as error:
        raise FirstAdjudicationObservationClientError(
            "first-adjudication observation input is invalid"
        ) from error
    ssh = shutil.which("ssh")
    if ssh is None:
        raise FirstAdjudicationObservationClientError("OpenSSH client is unavailable")
    try:
        with tempfile.TemporaryDirectory(
            prefix="cinegraph-speaker-observe-first-adjudication-"
        ) as directory:
            temp = Path(directory)
            if os.name != "nt":
                temp.chmod(0o700)
            ...
            try:
                ...
                raise FirstAdjudicationObservationClientError(
                    "SSH first-adjudication observation failed"
                ) from error
    except FirstAdjudicationObservationClientError:
        raise
    ...
    if (
        ...
        raise FirstAdjudicationObservationClientError(
            "remote first-adjudication observation was rejected"
        )
    try:
        return contract.parse_aggregate(stdout)
    except (TypeError, ValueError) as error:
        raise FirstAdjudicationObservationClientError(
            "remote first-adjudication observation response is invalid"
        ) from error
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (FirstAdjudicationObservationClientError, ValueError):
        sys.stderr.buffer.write(
            b'{"error":"speaker_review_first_adjudication_observation_rejected","status":"error"}\n'
        )
        return 2
