"""Pinned-SSH client for exactly adjudication part-four submission."""
...
class FourthAdjudicationSubmissionClientError(RuntimeError):
...
def _contract() -> ModuleType:
    try:
        module = import_module(
            "scripts.private_speaker_review_fourth_adjudication_submission_contract"
        )
        for name in (
            "COMMAND",
            "OPERATION",
        ...
        raise FourthAdjudicationSubmissionClientError("submission contract unavailable") from error


def ssh_arguments(*, ssh: str, identity: Path, known_hosts: Path, host: str) -> list[str]:
    policy = import_module("scripts.private_speaker_review_fourth_adjudication_host_contract")
    if policy.REVIEW_FOURTH_ADJUDICATION_COMMAND != _contract().COMMAND:
        raise FourthAdjudicationSubmissionClientError("submission host policy unavailable")
...
def _run_ssh(arguments: list[str], wire: Path) -> subprocess.CompletedProcess[bytes]:
    policy = import_module("scripts.private_speaker_review_fourth_adjudication_host_contract")
    timeout = (
        int(policy.REVIEW_FOURTH_ADJUDICATION_TIMEOUT_SECONDS)
        + int(policy.REVIEW_FOURTH_ADJUDICATION_KILL_AFTER_SECONDS)
        ...
        with wire.open("rb") as source:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                ...
                try:
                    ...
                    process.kill()
                    process.wait()
                    raise FourthAdjudicationSubmissionClientError("SSH submission failed") from error
                stdout = out.result(timeout=5)
                stderr = err.result(timeout=5)
        return subprocess.CompletedProcess(arguments, code, stdout, stderr)
    except FourthAdjudicationSubmissionClientError:
        raise
...
def submit_fourth_adjudication(
    *,
    ...
    try:
        ...
        if ssh is None:
            raise OSError
        with tempfile.TemporaryDirectory(prefix="cinegraph-speaker-fourth-adjudication-") as name:
        ...
        raise FourthAdjudicationSubmissionClientError(
            "remote fourth-adjudication submission failed"
        ) from error
    ...
    if (
        ...
        raise FourthAdjudicationSubmissionClientError(
            "remote fourth-adjudication submission rejected"
        )
    try:
        ...
        raise FourthAdjudicationSubmissionClientError(
            "remote fourth-adjudication response invalid"
        ) from error
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = submit_fourth_adjudication(
    ...
    except (FourthAdjudicationSubmissionClientError, ValueError):
        sys.stderr.write(
            '{"error":"speaker_review_fourth_adjudication_rejected","status":"error"}\n'
        )
        return 2
