"""Strict wire contract for the separately authorized fourth-part observer."""
...
COMMAND: Final = "speaker-review-observe-fourth-adjudication-v1"
PROTOCOL_VERSION: Final = 1
PURPOSE: Final = "speaker_review"
SEASON_NUMBER: Final = 2
OPERATION: Final = "observe_fourth_adjudication"
MAXIMUM_AUTHORIZED_COST_MICROUSD: Final = 5_000_000
PREDECESSOR_COMPLETED_PART_COUNT: Final = 3
OBSERVED_PART_NUMBER: Final = PREDECESSOR_COMPLETED_PART_COUNT + 1
...
RUN_STATUSES: Final = frozenset({"adjudication_submitted", "adjudication_part_completed", "failed"})
...
def _decode(raw: bytes, *, maximum: int) -> dict[str, object]:
    if (
        ...
        raise ValueError("invalid fourth-adjudication observation wire value")
    try:
        value = json.loads(
            raw.decode("utf-8"),
        ...
        raise ValueError("invalid fourth-adjudication observation wire value") from error
...
def validate_request(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        raise ValueError("invalid fourth-adjudication observation request")
...
def validate_aggregate(value: object, *, status: str | None = None) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != AGGREGATE_KEYS:
        raise ValueError("invalid fourth-adjudication observation aggregate")
    ...
    _cost(value.get("actual_primary_cost_microusd"), allow_zero=True)
    _cost(value.get("estimated_adjudication_cost_microusd"), allow_zero=True)
    for field in ("adjudication_part_count", "adjudication_completed_part_count"):
        if type(value.get(field)) is not int or value[field] < 0:
            raise ValueError("invalid fourth-adjudication observation aggregate")
    if (
        value["adjudication_part_count"] < OBSERVED_PART_NUMBER
        or value["adjudication_completed_part_count"] > value["adjudication_part_count"]
    ):
        raise ValueError("invalid fourth-adjudication observation aggregate")
    completed = value["adjudication_completed_part_count"]
    # This command is deliberately pinned to the Phase 75 checkpoint.  Waiting
    # and terminal failure retain the three completed parts; a successful
    # observation advances exactly one part to four.
    if expected in {"waiting", "reconciliation_required", "failed"} and completed != PREDECESSOR_COMPLETED_PART_COUNT:
        raise ValueError("invalid fourth-adjudication observation aggregate")
    if expected in {"observed", "already_observed"} and completed != OBSERVED_PART_NUMBER:
        raise ValueError("invalid fourth-adjudication observation aggregate")
    if (
        expected in {"waiting", "reconciliation_required"}
        and value["run_status"] != "adjudication_submitted"
    ):
        raise ValueError("invalid fourth-adjudication observation aggregate")
    if (
        expected in {"observed", "already_observed"}
        and value["run_status"] != "adjudication_part_completed"
    ):
        raise ValueError("invalid fourth-adjudication observation aggregate")
...
def is_reconciliation_error(error: BaseException) -> bool:
    ...
    return str(error) == SpeakerReviewErrorMessages.ADJUDICATION_OBSERVATION_RECONCILIATION_REQUIRED
