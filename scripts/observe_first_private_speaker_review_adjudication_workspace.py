"""Observe exactly the first adjudication Batch for a private review run.
...
from scripts import (
    private_speaker_review_first_adjudication_observation_contract as contract,  # noqa: E402
)
...
class FirstAdjudicationObservationWorkerError(RuntimeError):
...
def read_stable_openai_secret(path: Path = OPENAI_SECRET_PATH) -> str:
    ...
    try:
        ...
        raise FirstAdjudicationObservationWorkerError("observation secret unavailable") from error
    finally:
        if descriptor >= 0:
            os.close(descriptor)
    if (
        ...
        raise FirstAdjudicationObservationWorkerError("observation secret unavailable")
    try:
...
def request_from_environment(environment: Mapping[str, str] | None = None) -> dict[str, object]:
    ...
    try:
        ...
        raise FirstAdjudicationObservationWorkerError("observation request invalid") from error
...
def _parse_cost_cap(value: str) -> int:
    try:
        ...
        raise FirstAdjudicationObservationWorkerError("observation request invalid") from error
    if str(parsed) != value:
        raise FirstAdjudicationObservationWorkerError("observation request invalid")
...
def _cost_microusd(value: float) -> int:
    if not isinstance(value, (int, float)) or isinstance(value, bool) or not math.isfinite(value):
        raise FirstAdjudicationObservationWorkerError("observation cost invalid")
    try:
        micros = Decimal(str(value)) * Decimal(1_000_000)
    except (InvalidOperation, ValueError) as error:
        raise FirstAdjudicationObservationWorkerError("observation cost invalid") from error
...
def _run_directory(run_id: str, review_root: Path = PRIVATE_REVIEW_RUNS_ROOT) -> Path:
    ...
    try:
        if candidate.resolve(strict=False).parent != root:
            raise FirstAdjudicationObservationWorkerError("observation run unavailable")
    except OSError as error:
        raise FirstAdjudicationObservationWorkerError("observation run unavailable") from error
    return candidate
...
def _read_checkpoint_file(path: Path, maximum: int) -> bytes:
    ...
    try:
        ...
        raise FirstAdjudicationObservationWorkerError(
    ...
    if (
        ...
        raise FirstAdjudicationObservationWorkerError("observation checkpoint changed")
    return raw


def _expected_hash(value: str, label: str) -> str:
    if len(value) != 64 or any(character not in "0123456789abcdef" for character in value):
        raise FirstAdjudicationObservationWorkerError(f"observation {label} invalid")
    return value
...
def _expected_inventory_names(
    ...
    *,
    ...
    if observed:
        required.add("adjudication-part-0001-output.jsonl")
        optional.add("adjudication-part-0001-api-errors.jsonl")
...
def _validate_expected_checkpoint(
    ...
    if any(value is None for value in expected.values()):
        raise FirstAdjudicationObservationWorkerError("observation checkpoint invalid")
    if (
    ...
    for name in sorted(names):
        ...
        if total > 256 * 1024 * 1024:
            raise FirstAdjudicationObservationWorkerError("observation checkpoint unavailable")
        contents[name] = raw
...
def _aggregate(
    ...
    *,
    ...
    try:
        ...
        raise FirstAdjudicationObservationWorkerError("observation result invalid") from error
...
def _validate_state_transition(
    ...
    """Allow only the state mutation owned by one first adjudication observation."""
    ...
    if (
        ...
        raise FirstAdjudicationObservationWorkerError("observation result invalid")
...
def observe_first_adjudication(
    *,
    ...
    if state.status is SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED:
        ...
        raise FirstAdjudicationObservationWorkerError("observation run state invalid")
    ...
    if expected_total > cap or expected_total > configuration_cap:
        raise FirstAdjudicationObservationWorkerError("observation cost exceeds authorization")
    # A completed first adjudication part is the only safe replay state for this
    ...
    try:
        ...
        if _checkpoint_binding_present(values):
            returned_directory, observed = graph.observe_first_adjudication(
                canonical,
                verified_run_state=before,
            )
        else:
            returned_directory, observed = graph.observe_first_adjudication(canonical)
        ...
        raise FirstAdjudicationObservationWorkerError(
            "first adjudication observation failed"
        ) from error
...
def main() -> int:
    try:
        result = observe_first_adjudication()
        sys.stdout.buffer.write(contract.canonical_json(result))
        return 0
    except FirstAdjudicationObservationWorkerError:
        sys.stderr.write("error=speaker_review_first_adjudication_observation_failed\n")
        return 2
    except Exception:
        sys.stderr.write("error=speaker_review_first_adjudication_observation_failed\n")
        return 2
