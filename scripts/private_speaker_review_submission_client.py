"""Pinned-SSH workstation client for one authorized primary submission."""
...
class SpeakerReviewSubmissionClientError(RuntimeError):
    """A deliberately generic local or remote submission failure."""


def _host_contract() -> ModuleType:
    """Load host policy lazily so this client remains importable off the VPS."""

    module = import_module("scripts.private_speaker_review_submission_host_contract")
    for name in (
        "REVIEW_USER",
        "REVIEW_COMMAND",
        "TIMEOUT_SECONDS",
        ...
        if not hasattr(module, name):
            raise SpeakerReviewSubmissionClientError("submission host policy unavailable")
    return module


def _review_user() -> str:
    return str(_host_contract().REVIEW_USER)


def _review_command() -> str:
    return str(_host_contract().REVIEW_COMMAND)
...
def _regular(path: Path, *, private: bool = False) -> os.stat_result:
    try:
        ...
        raise SpeakerReviewSubmissionClientError("submission input is unavailable") from error
    if (
        ...
        raise SpeakerReviewSubmissionClientError("submission input is not a safe file")
    return result
...
def _validate_known_hosts(path: Path, host: str) -> None:
    ...
    try:
        ...
        raise SpeakerReviewSubmissionClientError("known-hosts input is invalid") from error
...
def _ssh_config_path(path: Path) -> str:
    ...
    if (
        ...
        raise SpeakerReviewSubmissionClientError("known-hosts input is invalid")
    return f'"{value}"'


def ssh_arguments(*, ssh: str, identity: Path, known_hosts: Path, host: str) -> list[str]:
    """Build a shell-free, host-key-pinned SSH invocation."""
...
def _run_ssh(arguments: list[str], wire: Path) -> subprocess.CompletedProcess[bytes]:
    ...
    timeout = (
        int(host_contract.TIMEOUT_SECONDS)
        + int(host_contract.KILL_AFTER_SECONDS)
        + int(host_contract.CLIENT_TIMEOUT_MARGIN_SECONDS)
    )
    try:
        with wire.open("rb") as source:
            ...
            if process.stdout is None or process.stderr is None:
                raise SpeakerReviewSubmissionClientError("SSH submission failed")
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                ...
                try:
                    ...
                    process.kill()
                    process.wait()
                    raise SpeakerReviewSubmissionClientError("SSH submission failed") from error
                stdout = stdout_future.result(timeout=5)
                stderr = stderr_future.result(timeout=5)
        return subprocess.CompletedProcess(arguments, returncode, stdout, stderr)
    except SpeakerReviewSubmissionClientError:
        raise
...
def submit_primary(
    *,
    ...
    """Authorize one remote primary submission without transferring a bundle."""
    ...
    try:
        ...
        raise SpeakerReviewSubmissionClientError("host input is invalid") from error
    _regular(identity, private=True)
    _validate_known_hosts(known_hosts, canonical_host)
    ssh = shutil.which("ssh")
    if ssh is None:
        raise SpeakerReviewSubmissionClientError("OpenSSH client is unavailable")
    with tempfile.TemporaryDirectory(prefix="cinegraph-speaker-submit-") as temp_name:
        temp = Path(temp_name)
        if os.name != "nt":
            temp.chmod(0o700)
    ...
    if (
        ...
        raise SpeakerReviewSubmissionClientError("remote submission was rejected")
    try:
        return contract.parse_aggregate(stdout)
    except (TypeError, ValueError) as error:
        raise SpeakerReviewSubmissionClientError("remote response is invalid") from error


submit_private_speaker_review = submit_primary
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (SpeakerReviewSubmissionClientError, ValueError):
        sys.stderr.buffer.write(
            b'{"error":"speaker_review_submission_rejected","status":"error"}\n'
        )
        return 2
