"""Strict wire contract for the separately authorized first-adjudication observer."""
...
COMMAND: Final = "speaker-review-observe-first-adjudication-v1"
PROTOCOL_VERSION: Final = 1
PURPOSE: Final = "speaker_review"
SEASON_NUMBER: Final = 2
OPERATION: Final = "observe_first_adjudication"
...
def _decode(raw: bytes, *, maximum: int) -> dict[str, object]:
    if (
        ...
        raise ValueError("invalid first-adjudication observation wire value")
    try:
        value = json.loads(
            raw.decode("utf-8"),
        ...
        raise ValueError("invalid first-adjudication observation wire value") from error
...
def validate_request(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        raise ValueError("invalid first-adjudication observation request")
