"""Strict wire contract for exactly private adjudication part-four submission."""
...
COMMAND: Final = "speaker-review-submit-fourth-adjudication-v1"
PROTOCOL_VERSION: Final = 1
PURPOSE: Final = "speaker_review"
SEASON_NUMBER: Final = 2
OPERATION: Final = "submit_fourth_adjudication"
...
SUBMITTED_PART_NUMBER: Final = PREDECESSOR_COMPLETED_PART_COUNT + 1
...
STATUSES: Final = frozenset(
    {"submitted", "already_submitted", "reconciliation_required", "all_parts_completed"}
)
...
def _decode(raw: bytes, *, maximum: int) -> dict[str, object]:
    if (
        ...
        raise ValueError("invalid fourth-adjudication wire value")
    try:
        value = json.loads(
            raw.decode("utf-8"),
        ...
        raise ValueError("invalid fourth-adjudication wire value") from error
...
def validate_request(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        raise ValueError("invalid fourth-adjudication request")
...
def validate_aggregate(value: object, *, status: str | None = None) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != AGGREGATE_KEYS:
        raise ValueError("invalid fourth-adjudication aggregate")
    ...
    if (
        ...
        or completed_count != PREDECESSOR_COMPLETED_PART_COUNT
        or part_count < completed_count
        or submitted_count not in (0, 1)
    ):
        raise ValueError("invalid fourth-adjudication aggregate")
    if expected in {"submitted", "already_submitted", "reconciliation_required"} and part_count <= completed_count:
        raise ValueError("invalid fourth-adjudication aggregate")
    ...
    if expected == "reconciliation_required" and submitted_count != 0:
        raise ValueError("invalid fourth-adjudication aggregate")
    if expected == "all_parts_completed" and (
        part_count != PREDECESSOR_COMPLETED_PART_COUNT
        or completed_count != PREDECESSOR_COMPLETED_PART_COUNT
        or submitted_count != 0
        or value["run_status"] != "adjudication_part_completed"
    ):
        raise ValueError("invalid fourth-adjudication aggregate")
    return dict(value)
