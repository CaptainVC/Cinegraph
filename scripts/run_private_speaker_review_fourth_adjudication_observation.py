"""Root-only coordinator for observing exactly adjudication part four.

It authenticates the complete Phase 74/75 evidence chain, performs at most one
...
from scripts import (  # noqa: E402
    private_speaker_review_fourth_adjudication_observation_contract as contract,
)
from scripts import (  # noqa: E402
    private_speaker_review_fourth_adjudication_observation_host_contract as host,
)
from scripts import (  # noqa: E402
    private_speaker_review_fourth_adjudication_submission_contract as submit_contract,
)
...
try:  # Application/provider imports remain outside the privileged root boundary.
    ...
    from scripts import (  # noqa: E402
        observe_fourth_private_speaker_review_adjudication_workspace as worker,
    )
    from scripts import (  # noqa: E402
        run_private_speaker_review_fourth_adjudication as submit,
    )
    from scripts import (  # noqa: E402
    ...
    # The host path is stdlib-only under ``-I -S``.  Phase 75 supplies a
    ...
    submit = importlib.import_module("scripts.run_private_speaker_review_fourth_adjudication")
...
class FourthAdjudicationObservationError(RuntimeError):
...
SUBMISSION_RECEIPTS_ROOT: Final = host.REVIEW_FOURTH_ADJUDICATION_SUBMISSION_RECEIPTS_ROOT
OBSERVATION_RECEIPTS_ROOT: Final = host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_RECEIPTS_ROOT
...
REQUEST: Final = "adjudication-part-0004-requests.jsonl"
SUBMISSION_INTENT: Final = ".adjudication-part-0004-submission-intent.json"
SUBMISSION_COMPLETED: Final = ".adjudication-part-0004-submission-completed.json"
OBSERVATION_OUTPUTS: Final = frozenset(
    {
        "adjudication-part-0004-output.jsonl",
        "adjudication-part-0004-api-errors.jsonl",
    }
)
...
_BINDING_KEYS = frozenset(
    {
        ...
        "fourth_adjudication_submission_intent_sha256",
        "fourth_adjudication_submission_receipt_sha256",
        "operation",
...
def _micros(value: object) -> int:
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise FourthAdjudicationObservationError("observation cost invalid")
    try:
        decimal = Decimal(str(value))
    except (InvalidOperation, ValueError) as error:
        raise FourthAdjudicationObservationError("observation cost invalid") from error
...
def _directory(path: Path, *, mode: int, owner: tuple[int, int]) -> None:
    try:
        ...
        raise FourthAdjudicationObservationError("observation evidence unavailable") from error
...
def _stable(path: Path, *, maximum: int, mode: int, owner: tuple[int, int]) -> bytes:
    ...
    if (
        ...
        raise FourthAdjudicationObservationError("observation evidence changed")
    return raw
...
def _decode(raw: bytes) -> dict[str, object]:
    try:
        ...
        raise FourthAdjudicationObservationError("observation evidence invalid") from error
    if not isinstance(value, dict) or _canonical(value) != raw:
        raise FourthAdjudicationObservationError("observation evidence invalid")
    return value
...
def _read_request(stream: BinaryIO) -> dict[str, object]:
    ...
    if stream.read(1):
        raise FourthAdjudicationObservationError("invalid observation request")
    try:
        return contract.parse_request(raw)
    except (TypeError, ValueError) as error:
        raise FourthAdjudicationObservationError("invalid observation request") from error
...
def _validate_authorization(request: Mapping[str, object]) -> str:
    ...
    try:
        ...
        raise FourthAdjudicationObservationError("observation authorization invalid") from error
    return _sha(raw)
...
def _run_directory(request: Mapping[str, object]) -> Path:
    ...
    if path.resolve(strict=False).parent != runs.resolve(strict=True):
        raise FourthAdjudicationObservationError("observation run invalid")
...
def _inventory(
    ...
    try:
        # Phase 75 exposes a complete, flat inventory together with the
        ...
        raise FourthAdjudicationObservationError("observation inventory invalid") from error
    if (
        canonical != run
        ...
        raise FourthAdjudicationObservationError("observation run state invalid")
...
def _validate_submitted_state(state: Mapping[str, object]) -> None:
    try:
        ...
        raise FourthAdjudicationObservationError("observation checkpoint invalid") from error


def _phase75_request(
    request: Mapping[str, object], intent: Mapping[str, object]
) -> tuple[dict[str, object], bytes, str]:
    """Load the exact Phase 75 submission authorization named by its intent."""
...
def _validate_phase75_chain(
    request: Mapping[str, object],
    run: Path,
    contents: Mapping[str, bytes],
    state: submit.SpeakerReviewRunState,
) -> tuple[dict[str, object], str, str, str, int]:
    """Authenticate the complete Phase 75 submission predecessor."""
    ...
    predecessor_request, predecessor_raw, predecessor_auth_sha = _phase75_request(request, intent)
    ...
    (
        ...
        predecessor_actual,
    ) = submit._validate_phase74_predecessor(
        predecessor_request,
        run,
    ...
    submit.worker._validate_replay_evidence(run, contents, state)
    if (
        intent.get("fourth_adjudication_part_number") != contract.OBSERVED_PART_NUMBER
        ...
        or result["estimated_adjudication_cost_microusd"] != intent["estimated_adjudication_cost_microusd"]
    ...
    return predecessor_preparation, predecessor_preparation_sha, intent_sha, receipt_sha, estimated
...
def _validate_submission_predecessor(
    ...
    """Validate Phase 75's root receipt and complete predecessor chain."""
    try:
        return _validate_phase75_chain(request, run, contents, state)
    except FourthAdjudicationObservationError:
        raise
    except Exception as error:
        raise FourthAdjudicationObservationError(
            "fourth-adjudication submission evidence invalid"
        ) from error
...
def _root_binding(
    ...
    return {
        ...
        "fourth_adjudication_submission_intent_sha256": submission_intent_sha256,
        "fourth_adjudication_submission_receipt_sha256": submission_receipt_sha256,
...
def _validate_intent(
    ...
    *,
    ...
    if not isinstance(value, dict) or set(value) != _BINDING_KEYS:
        raise FourthAdjudicationObservationError("observation intent invalid")
...
def _validate_predecessors_from_intent(
    ...
    # the intent, so the Phase 75 receipt is authenticated against exactly the
    ...
    try:
        ...
        (
            ...
            _estimate,
        ) = _validate_phase75_chain(request, run, before, before_model)
    except FourthAdjudicationObservationError:
        raise
    except Exception as error:
        raise FourthAdjudicationObservationError(
    ...
    if (
        ...
        or intent.get("fourth_adjudication_submission_intent_sha256") != submission_intent_sha
        or intent.get("fourth_adjudication_submission_receipt_sha256") != submission_receipt_sha
        or intent.get("estimated_adjudication_cost_microusd") != _estimate
        ...
        or preparation_value.get("config_sha") != intent.get("configuration_sha256")
    ):
        raise FourthAdjudicationObservationError("observation predecessor evidence changed")
...
def _validate_checkpoint_against_intent(
    ...
    if (
        ...
        raise FourthAdjudicationObservationError("observation checkpoint changed")
...
def _aggregate(
    ...
    try:
        ...
        raise FourthAdjudicationObservationError("observation aggregate invalid") from error
...
def _validate_final_receipt(
    ...
    *,
    ...
    if (
        ...
        raise FourthAdjudicationObservationError("observation receipt invalid")
...
def _container_identity_is_exact(
    ...
    try:
        ...
        image_inspected = subprocess.run(
            ...
            shell=False,
            check=False,
            timeout=host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_KILL_AFTER_SECONDS,
        )
        inspected = subprocess.run(
            [
                ...
                "--format",
                "{{json .}}",
                host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_CONTAINER_NAME,
            ],
            cwd=RELEASE_ROOT,
            env=_safe_env(),
            stdin=subprocess.DEVNULL,
    ...
    if (
        value.get("Name") != f"/{host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_CONTAINER_NAME}"
        or config.get("Image") != expected_image
        or config.get("User") != f"{WORKER_UID}:{WORKER_GID}"
        or config.get("WorkingDir") != host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_CONTAINER_WORKDIR
        or config.get("Cmd") != list(host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_CONTAINER_COMMAND)
        or not isinstance(labels, dict)
        or labels.get("com.docker.compose.service")
        != host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_COMPOSE_SERVICE
        or labels.get("com.docker.compose.oneoff") != "True"
        or labels.get("com.docker.compose.project")
        != host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_COMPOSE_PROJECT
        ...
        or set(networks) != {host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_NETWORK}
    ):
        return False
    ...
    return (
        ...
        and destinations.get(host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_SECRET_TARGET, ("", True))[
            0
        ]
        != ""
        and destinations.get(host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_SECRET_TARGET, ("", True))[
        ...
        is False
        and destinations.get(host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_TMP_TARGET) == ("", True)
        and set(destinations)
        == {
            expected_destination,
            host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_SECRET_TARGET,
            host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_TMP_TARGET,
        }
    )
...
def _cleanup_worker(
    ...
    try:
        subprocess.run(
            ["docker", "rm", "--force", host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_CONTAINER_NAME],
            cwd=RELEASE_ROOT,
            env=_safe_env(),
            stdin=subprocess.DEVNULL,
...
def _terminate_worker(process: subprocess.Popen[bytes]) -> None:
    ...
    if os.name == "posix":
        try:
            ...
            pass
        try:
            process.wait(timeout=host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_KILL_AFTER_SECONDS)
            return
...
def _worker_args(
    ...
    args = [
        ...
        os.fspath(ENV_FILE),
        "--profile",
        host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_COMPOSE_PROFILE,
        "-f",
    ...
    args.append(host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_COMPOSE_SERVICE)
    return args
...
def _run_worker(
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
            ...
            code = process.wait(
                timeout=host.REVIEW_FOURTH_ADJUDICATION_OBSERVATION_TIMEOUT_SECONDS - 60
            )
        ...
        raise FourthAdjudicationObservationError("observation worker failed") from error
    finally:
...
def _write_receipt(path: Path, value: Mapping[str, object]) -> None:
    ...
    if len(raw) > MAX_RECORD_BYTES:
        raise FourthAdjudicationObservationError("observation receipt too large")
    pending = path.with_name(f".{path.name}.pending")
    if os.path.lexists(path):
        submit._repair_linked_publication(path)
        existing, _ = _read_record(path)
        if existing != dict(value):
            raise FourthAdjudicationObservationError("observation receipt conflict")
        return
    if os.path.lexists(pending):
        ...
        try:
            ...
            raise FourthAdjudicationObservationError("observation receipt unavailable") from error
        finally:
            if descriptor >= 0:
                os.close(descriptor)
    try:
        ...
        existing, _ = _read_record(path)
        if existing != dict(value):
            raise FourthAdjudicationObservationError("observation receipt conflict") from None
...
def _post_validate(
    ...
    if set(before_state) != set(after_state) or any(
        ...
        raise FourthAdjudicationObservationError("observation immutable state changed")
    if any(
        name != STATE and (name not in after or _sha(after[name]) != _sha(raw))
        for name, raw in before.items()
    ):
        raise FourthAdjudicationObservationError("observation immutable evidence changed")
    ...
    if status == "observed":
        if (
            ...
            != contract.PREDECESSOR_COMPLETED_PART_COUNT
            or f"adjudication-part-{contract.OBSERVED_PART_NUMBER:04d}-output.jsonl" not in after
            or not observation._safe_text(after_state.get("updated_at"), maximum=128)
        ):
            raise FourthAdjudicationObservationError("observation post-state invalid")
    elif status == "failed":
        if (
        ...
        raise FourthAdjudicationObservationError("observation worker status invalid")
...
def _post_inventory(run: Path):
    ...
    if first != second:
        raise FourthAdjudicationObservationError("observation post-inventory changed")
    return second
...
def process_request(request: Mapping[str, object]) -> dict[str, object]:
    ...
    try:
        # A crash after hard-link publication can leave the final record and
        # its staging name linked to the same authenticated inode. Repair that
        # exact safe shape before classifying any remaining pending evidence.
        if intent_exists and intent_pending_exists:
            submit._repair_linked_publication(intent_path)
            intent_pending_exists = False
        if receipt_exists and receipt_pending_exists:
            submit._repair_linked_publication(receipt_path)
            receipt_pending_exists = False
    except Exception as error:
        raise FourthAdjudicationObservationError(
            "observation receipt state ambiguous"
        ) from error
    if receipt_exists and not intent_exists:
        raise FourthAdjudicationObservationError("orphan observation receipt")
    # A pending intent is safely repaired only after its binding is rebuilt
    # below. A pending final receipt is never safe to replay or overwrite:
    # it may represent an interrupted publication after an unknown provider
    # transition and therefore requires explicit reconciliation.
    if receipt_pending_exists or (intent_pending_exists and intent_exists):
        raise FourthAdjudicationObservationError("observation receipt state ambiguous")
    ...
    if not (submitted or observed or failed):
        raise FourthAdjudicationObservationError("observation checkpoint invalid")

    if observed or failed:
        if not intent_exists:
            raise FourthAdjudicationObservationError("observation intent missing")
        ...
        if terminal_status in {"observed", "failed"} and not receipt_exists:
            ...
            raise FourthAdjudicationObservationError("observation reconciliation required")
    ...
    if (
        ...
        raise FourthAdjudicationObservationError("observation cost exceeds authorization")
    ...
    if intent_exists:
        ...
        if intent != binding:
            raise FourthAdjudicationObservationError("observation intent binding changed")
    else:
        if receipt_exists:
            raise FourthAdjudicationObservationError("orphan observation receipt")
        intent = binding
    ...
    if receipt_exists:
        raise FourthAdjudicationObservationError("observation receipt state invalid")
    ...
    if status not in {"waiting", "observed", "failed", "reconciliation_required"}:
        raise FourthAdjudicationObservationError("observation worker result invalid")
    expected_completed = (
        contract.OBSERVED_PART_NUMBER
        if status == "observed"
        else contract.PREDECESSOR_COMPLETED_PART_COUNT
    )
    expected_run_status = (
    ...
    if preparation._active_binding() != (
        ...
        raise FourthAdjudicationObservationError("active runtime changed")
...
def _require_root() -> None:
    if (
        ...
        raise FourthAdjudicationObservationError("invalid observation caller")
...
def main() -> int:
    try:
        ...
        sys.stderr.write("error=speaker_review_fourth_adjudication_observation_rejected\n")
        return 2
