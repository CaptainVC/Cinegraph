"""Root-only coordinator for observing exactly adjudication part two.

It authenticates the complete Phase 70/71 evidence chain, performs at most one
...
from scripts import (  # noqa: E402
    private_speaker_review_next_adjudication_observation_contract as contract,
)
from scripts import (  # noqa: E402
    private_speaker_review_next_adjudication_observation_host_contract as host,
)
from scripts import (  # noqa: E402
    private_speaker_review_next_adjudication_submission_contract as submit_contract,
)

try:  # Keep application/provider imports inside the Compose worker boundary.
    from scripts import (  # noqa: E402
        run_private_speaker_review_first_adjudication_observation as phase70,
    )
    from scripts import run_private_speaker_review_next_adjudication as submit  # noqa: E402
    from scripts import run_private_speaker_review_next_primary as preparation  # noqa: E402
    from scripts import run_private_speaker_review_observation as observation  # noqa: E402
    from scripts import (  # noqa: E402
    ...
    # All host predecessors below are stdlib-only under ``-I -S``. Reuse
    # their complete authentication, inventory, and immutable-source checks;
    # only the provider worker remains outside this privileged process.
    phase70 = importlib.import_module(
        "scripts.run_private_speaker_review_first_adjudication_observation"
    )
    submit = importlib.import_module("scripts.run_private_speaker_review_next_adjudication")
    ...
    worker = types.SimpleNamespace(
        _expected_inventory_names=lambda state, **kwargs: submit._expected_inventory_names(state),
        _validate_checkpoint_shape=lambda state, *, submitted: submit._validate_state_shape(state),
        _validate_completed_parts=lambda run, contents, state, count: submit._validate_completed_evidence(run, contents, state),
        _validate_journal=lambda run, contents, state, part, **kwargs: submit._stdlib_journal(contents, state, part, **kwargs),
    )


class NextAdjudicationObservationError(RuntimeError):
...
SUBMISSION_RECEIPTS_ROOT: Final = host.REVIEW_NEXT_ADJUDICATION_SUBMISSION_RECEIPTS_ROOT
OBSERVATION_RECEIPTS_ROOT: Final = host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_RECEIPTS_ROOT
...
REQUEST: Final = "adjudication-part-0002-requests.jsonl"
SUBMISSION_INTENT: Final = ".adjudication-part-0002-submission-intent.json"
SUBMISSION_COMPLETED: Final = ".adjudication-part-0002-submission-completed.json"
OBSERVATION_OUTPUTS: Final = frozenset(
    {
        "adjudication-part-0002-output.jsonl",
        "adjudication-part-0002-api-errors.jsonl",
    }
)
...
_BINDING_KEYS = frozenset(
    {
        ...
        "next_adjudication_submission_intent_sha256",
        "next_adjudication_submission_receipt_sha256",
        "operation",
...
def _micros(value: object) -> int:
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise NextAdjudicationObservationError("observation cost invalid")
    try:
        decimal = Decimal(str(value))
    except (InvalidOperation, ValueError) as error:
        raise NextAdjudicationObservationError("observation cost invalid") from error
...
def _directory(path: Path, *, mode: int, owner: tuple[int, int]) -> None:
    try:
        ...
        raise NextAdjudicationObservationError("observation evidence unavailable") from error
...
def _stable(path: Path, *, maximum: int, mode: int, owner: tuple[int, int]) -> bytes:
    ...
    if (
        ...
        raise NextAdjudicationObservationError("observation evidence changed")
    return raw
...
def _decode(raw: bytes) -> dict[str, object]:
    try:
        ...
        raise NextAdjudicationObservationError("observation evidence invalid") from error
    if not isinstance(value, dict) or _canonical(value) != raw:
        raise NextAdjudicationObservationError("observation evidence invalid")
    return value
...
def _read_request(stream: BinaryIO) -> dict[str, object]:
    ...
    if stream.read(1):
        raise NextAdjudicationObservationError("invalid observation request")
    try:
        return contract.parse_request(raw)
    except (TypeError, ValueError) as error:
        raise NextAdjudicationObservationError("invalid observation request") from error
...
def _validate_authorization(request: Mapping[str, object]) -> str:
    ...
    try:
        ...
        raise NextAdjudicationObservationError("observation authorization invalid") from error
    return _sha(raw)
...
def _run_directory(request: Mapping[str, object]) -> Path:
    ...
    if path.resolve(strict=False).parent != runs.resolve(strict=True):
        raise NextAdjudicationObservationError("observation run invalid")
...
def _inventory(
    ...
    try:
        contents, _, _, _, extra = phase70.submit._inventory(run)
        state_payload = extra["state"]
        canonical, state_model = submit.load_validated_run_state(
            run,
            submit.DEFAULT_SPEAKER_REVIEW_CONFIGURATION,
        )
        if not isinstance(state_payload, dict):
            raise ValueError("state payload")
        # Phase 71 has already authenticated and canonicalized this state;
        # keep the observation boundary's public state as a plain dict so
        # this root process does not need the workflow model at all.
        state = dict(state_payload)
        status = getattr(state_model.status, "value", state_model.status)
        observed = (
            status == "adjudication_part_completed"
            and state_model.adjudication_completed_part_count >= 2
        )
        if status == "failed":
        ...
        raise NextAdjudicationObservationError("observation inventory invalid") from error
    if (
        canonical != run
        ...
        or any(name.endswith("/") for name in extra["derived"])
    ):
        raise NextAdjudicationObservationError("observation run state invalid")
...
def _validate_submitted_state(state: Mapping[str, object]) -> None:
    try:
        ...
        if (
            ...
            or state["adjudication_part_count"] <= 1
            ...
            or len(set(batch_ids)) != 2
            or len(set(input_ids)) != 2
            or not all(observation._safe_text(value) for value in (*batch_ids, *input_ids))
        ):
            raise ValueError
    except (KeyError, TypeError, ValueError) as error:
        raise NextAdjudicationObservationError("observation checkpoint invalid") from error
...
def _validate_submission_predecessor(
    ...
    """Validate Phase 71's root receipt and full submitted checkpoint."""
    try:
        _validate_submitted_state(state.to_dict())
        preparation_value, preparation_sha = preparation._validate_preparation(request)
        phase70.submit.phase68._source_workspace(request)
        _directory(SUBMISSION_RECEIPTS_ROOT, mode=0o700, owner=(ROOT_UID, ROOT_GID))
        intent, intent_sha = submit._record(SUBMISSION_RECEIPTS_ROOT / f"{request['run_id']}.intent.json")
        ...
        if set(intent) != submit._ROOT_INTENT_KEYS or set(receipt) != submit._ROOT_RECEIPT_KEYS:
            raise ValueError
        predecessor_auth = intent.get("authorization_id")
        if not isinstance(predecessor_auth, str):
            raise ValueError
        predecessor_raw = _stable(
            AUTHORIZATION_ROOT / f"{predecessor_auth}.json",
            maximum=submit_contract.REQUEST_MAX_BYTES,
            mode=0o600,
            owner=(ROOT_UID, ROOT_GID),
        )
        predecessor_request = submit_contract.parse_request(predecessor_raw)
        if predecessor_request["archive_sha256"] != request["archive_sha256"] or predecessor_request["run_id"] != request["run_id"]:
            raise ValueError
        # Revalidate the complete Phase 70 chain (including preparation,
        # release/image/configuration and its post-inventory snapshot) before
        # trusting the Phase 71 submit-next receipt.
        (
            _,
            _,
            phase70_intent_sha,
            ...
            _,
            _,
        ) = submit._validate_phase70_predecessor(
            predecessor_request,
            run,
            contents,
            state,
        ...
        if (
            intent.get("phase70_observation_intent_sha256") != phase70_intent_sha
            or intent.get("phase70_observation_receipt_sha256")
            != phase70_receipt_sha
            ...
            raise ValueError
        worker._validate_checkpoint_shape(state, submitted=True)
        worker._validate_completed_parts(run, contents, state, 1)
        worker._validate_journal(
            run,
            contents,
            state,
            2,
        ...
        if type(estimated) is not int:
            raise ValueError
        if _sha(predecessor_raw) != intent["authorization_sha256"]:
            raise ValueError
        return preparation_value, preparation_sha, intent_sha, receipt_sha, estimated
    except NextAdjudicationObservationError:
        raise
    except Exception as error:
        raise NextAdjudicationObservationError("next-adjudication submission evidence invalid") from error
...
def _root_binding(
    ...
    return {
        ...
        "next_adjudication_submission_intent_sha256": submission_intent_sha256,
        "next_adjudication_submission_receipt_sha256": submission_receipt_sha256,
...
def _validate_intent(
    ...
    *,
    ...
    if not isinstance(value, dict) or set(value) != _BINDING_KEYS:
        raise NextAdjudicationObservationError("observation intent invalid")
    ...
    if (
        ...
        or value.get("adjudication_completed_part_count") != 1
        or value.get("observed_part_number") != 2
...
def _validate_predecessors_from_intent(
    ...
    try:
        _, preparation_sha = preparation._validate_preparation(request)
        phase70.submit.phase68._source_workspace(request)
        submission_intent, submission_intent_sha = submit._record(
            SUBMISSION_RECEIPTS_ROOT / f"{request['run_id']}.intent.json"
        )
        submission_receipt, submission_receipt_sha = submit._record(
            SUBMISSION_RECEIPTS_ROOT / f"{request['run_id']}.json"
        )
        predecessor_auth = submission_intent.get("authorization_id")
        if not isinstance(predecessor_auth, str):
            raise ValueError
        predecessor_raw = _stable(
            AUTHORIZATION_ROOT / f"{predecessor_auth}.json",
        ...
        predecessor_request = submit_contract.parse_request(predecessor_raw)
        if (
            predecessor_request["archive_sha256"] != request["archive_sha256"]
            or predecessor_request["run_id"] != request["run_id"]
        ):
            raise ValueError
        (
            _,
        ...
        submit._validate_binding(submission_intent, predecessor_request)
        result = submit._validate_receipt(
            submission_receipt,
        ...
        if (
            ...
            or result["submitted_part_count"] != 1
            or result["adjudication_completed_part_count"] != 1
            or result["adjudication_part_count"]
            != intent["adjudication_part_count"]
            or result["actual_primary_cost_microusd"] != prior_actual
            or result["actual_primary_cost_microusd"]
            != intent["actual_primary_cost_microusd"]
            or result["estimated_adjudication_cost_microusd"]
            != intent["estimated_adjudication_cost_microusd"]
            or _sha(predecessor_raw)
            != submission_intent["authorization_sha256"]
            or submission_intent.get("phase70_observation_intent_sha256")
            != phase70_intent_sha
            or submission_intent.get("phase70_observation_receipt_sha256")
            != phase70_receipt_sha
        ):
            raise ValueError
    except Exception as error:
        raise NextAdjudicationObservationError(
            "observation predecessor evidence invalid"
        ) from error
    if (
        intent.get("prep_receipt_sha256") != preparation_sha
        or intent.get("next_adjudication_submission_intent_sha256") != submission_intent_sha
        or intent.get("next_adjudication_submission_receipt_sha256") != submission_receipt_sha
        ...
        raise NextAdjudicationObservationError("observation predecessor evidence changed")
...
def _validate_checkpoint_against_intent(
    ...
    if (
        ...
        raise NextAdjudicationObservationError("observation checkpoint changed")
...
def _aggregate(
    ...
    try:
        ...
        raise NextAdjudicationObservationError("observation aggregate invalid") from error
...
def _validate_final_receipt(
    ...
    *,
    ...
    if (
        ...
        raise NextAdjudicationObservationError("observation receipt invalid")
...
def _container_identity_is_exact(runs: Path, run_id: str | None = None) -> bool:
    try:
        ...
        inspected = subprocess.run(
            [
                ...
                "--format",
                "{{json .}}",
                host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_CONTAINER_NAME,
            ],
            cwd=RELEASE_ROOT,
            env=_safe_env(),
            ...
            shell=False,
            check=False,
            timeout=host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_KILL_AFTER_SECONDS,
        )
    ...
    if (
        value.get("Name") != f"/{host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_CONTAINER_NAME}"
        or config.get("Image") != expected_image
        or config.get("User") != f"{WORKER_UID}:{WORKER_GID}"
        or config.get("WorkingDir") != host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_CONTAINER_WORKDIR
        or config.get("Cmd") != list(host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_CONTAINER_COMMAND)
        or not isinstance(labels, dict)
        or labels.get("com.docker.compose.service")
        != host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_COMPOSE_SERVICE
        or labels.get("com.docker.compose.oneoff") != "True"
        or labels.get("com.docker.compose.project")
        != host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_COMPOSE_PROJECT
        ...
        or set(networks) != {host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_NETWORK}
    ):
        return False
    ...
    expected_destination = WORKER_MOUNT
    ...
    return (
        ...
        and destinations.get(host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_SECRET_TARGET, ("", True))[
            0
        ]
        != ""
        and destinations.get(host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_SECRET_TARGET, ("", True))[
        ...
        is False
        and destinations.get(host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_TMP_TARGET) == ("", True)
        and set(destinations)
        == {
            expected_destination,
            host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_SECRET_TARGET,
            host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_TMP_TARGET,
        }
    )
...
def _cleanup_worker(runs: Path, run_id: str | None = None) -> None:
    ...
    try:
        subprocess.run(
            ["docker", "rm", "--force", host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_CONTAINER_NAME],
            cwd=RELEASE_ROOT,
            env=_safe_env(),
            stdin=subprocess.DEVNULL,
...
def _terminate_worker(process: subprocess.Popen[bytes]) -> None:
    ...
    if os.name == "posix":
        try:
            ...
            pass
        try:
            process.wait(timeout=host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_KILL_AFTER_SECONDS)
            return
...
def _worker_args(
    ...
    args = [
        ...
        os.fspath(ENV_FILE),
        "--profile",
        host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_COMPOSE_PROFILE,
        "-f",
    ...
    args.append(host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_COMPOSE_SERVICE)
    return args
...
def _run_worker(
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
            ...
            code = process.wait(
                timeout=host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_TIMEOUT_SECONDS - 60
            )
        ...
        raise NextAdjudicationObservationError("observation worker failed") from error
    finally:
...
def _write_receipt(path: Path, value: Mapping[str, object]) -> None:
    ...
    if len(raw) > MAX_RECORD_BYTES:
        raise NextAdjudicationObservationError("observation receipt too large")
    pending = path.with_name(f".{path.name}.pending")
    if os.path.lexists(path):
        submit._repair_linked_publication(path)
        existing, _ = _read_record(path)
        if existing != dict(value):
            raise NextAdjudicationObservationError("observation receipt conflict")
        return
    if os.path.lexists(pending):
        ...
        try:
            ...
            raise NextAdjudicationObservationError("observation receipt unavailable") from error
        finally:
            if descriptor >= 0:
                os.close(descriptor)
    try:
        ...
        existing, _ = _read_record(path)
        if existing != dict(value):
            raise NextAdjudicationObservationError("observation receipt conflict") from None
...
def _post_validate(
    ...
    if set(before_state) != set(after_state) or any(
        ...
        raise NextAdjudicationObservationError("observation immutable state changed")
    if any(
        name != STATE and (name not in after or _sha(after[name]) != _sha(raw))
        for name, raw in before.items()
    ):
        raise NextAdjudicationObservationError("observation immutable evidence changed")
    ...
    if status == "observed":
        if (
            ...
            or before_state.get("adjudication_completed_part_count") != 1
            or "adjudication-part-0002-output.jsonl" not in after
            or not observation._safe_text(after_state.get("updated_at"), maximum=128)
        ):
            raise NextAdjudicationObservationError("observation post-state invalid")
    elif status == "failed":
        if (
        ...
        raise NextAdjudicationObservationError("observation worker status invalid")
...
def _post_inventory(run: Path):
    ...
    if first != second:
        raise NextAdjudicationObservationError("observation post-inventory changed")
    return second
...
def process_request(request: Mapping[str, object]) -> dict[str, object]:
    ...
    if receipt_exists and not intent_exists:
        raise NextAdjudicationObservationError("orphan observation receipt")
    ...
    observed = (
        ...
        and state.get("adjudication_completed_part_count") == 2
    )
    failed = state.get("status") == "failed" and state.get("adjudication_completed_part_count") == 1
    if not (submitted or observed or failed):
        raise NextAdjudicationObservationError("observation checkpoint invalid")

    if observed or failed:
        if not intent_exists:
            raise NextAdjudicationObservationError("observation intent missing")
        ...
        if terminal_status in {"observed", "failed"} and not receipt_exists:
            ...
            raise NextAdjudicationObservationError("observation reconciliation required")
    ...
    if (
        ...
        raise NextAdjudicationObservationError("observation cost exceeds authorization")
    ...
    if intent_exists:
        ...
        if intent != binding:
            raise NextAdjudicationObservationError("observation intent binding changed")
    else:
        if receipt_exists:
            raise NextAdjudicationObservationError("orphan observation receipt")
        intent = binding
    ...
    if receipt_exists:
        raise NextAdjudicationObservationError("observation receipt state invalid")
    ...
    if status not in {"waiting", "observed", "failed", "reconciliation_required"}:
        raise NextAdjudicationObservationError("observation worker result invalid")
    expected_completed = 2 if status == "observed" else 1
    expected_run_status = (
    ...
    if preparation._active_binding() != (
        ...
        raise NextAdjudicationObservationError("active runtime changed")
    phase70.submit.phase68._source_workspace(request)
...
def _require_root() -> None:
    if (
        ...
        raise NextAdjudicationObservationError("invalid observation caller")
...
def main() -> int:
    try:
        ...
        sys.stderr.write("error=speaker_review_next_adjudication_observation_rejected\n")
        return 2
