"""Strict wire contract for the separately authorized next-part observer."""
...
COMMAND: Final = "speaker-review-observe-next-adjudication-v1"
PROTOCOL_VERSION: Final = 1
PURPOSE: Final = "speaker_review"
SEASON_NUMBER: Final = 2
OPERATION: Final = "observe_next_adjudication"
...
def _decode(raw: bytes, *, maximum: int) -> dict[str, object]:
    if (
        ...
        raise ValueError("invalid next-adjudication observation wire value")
    try:
        value = json.loads(
            raw.decode("utf-8"),
        ...
        raise ValueError("invalid next-adjudication observation wire value") from error
...
def validate_request(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        raise ValueError("invalid next-adjudication observation request")
...
def validate_aggregate(value: object, *, status: str | None = None) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != AGGREGATE_KEYS:
        raise ValueError("invalid next-adjudication observation aggregate")
    ...
    for field in ("adjudication_part_count", "adjudication_completed_part_count"):
        if type(value.get(field)) is not int or value[field] < 0:
            raise ValueError("invalid first-adjudication observation aggregate")
    if (
    ...
    # A next-part observer is generic at the wire level: the root boundary
    # constrains k to one (part two), while the application primitive may
    # safely use any positive completed count below the total.
    if expected in {"waiting", "reconciliation_required", "failed"} and completed <= 0:
        raise ValueError("invalid next-adjudication observation aggregate")
    if expected in {"observed", "already_observed"} and completed <= 1:
        raise ValueError("invalid next-adjudication observation aggregate")
    if (
