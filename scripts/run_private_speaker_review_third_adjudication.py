"""Root-only coordinator for exactly adjudication part-three submission.

The coordinator is the host half of the Phase 73 boundary. It authenticates
one canonical request, verifies the complete Phase 72 observation chain and
the digest-selected run, then starts one constrained Compose worker for exactly
adjudication part three. The application transition remains generic, but this
...
from scripts import (  # noqa: E402
    private_speaker_review_next_adjudication_observation_contract as phase72_contract,
)
from scripts import (  # noqa: E402
    private_speaker_review_next_adjudication_observation_host_contract as phase72_host,
)
from scripts import (  # noqa: E402
    private_speaker_review_third_adjudication_host_contract as host,
)
from scripts import (  # noqa: E402
    private_speaker_review_third_adjudication_submission_contract as contract,
)
...
try:  # Application/state and provider worker imports are never required at root.
    ...
    from scripts import (  # noqa: E402
        run_private_speaker_review_next_adjudication_observation as phase72,
    )
    from scripts import run_private_speaker_review_next_primary as preparation  # noqa: E402
    from scripts import (  # noqa: E402
        submit_third_private_speaker_review_adjudication_workspace as worker,
    )
    ...
    phase72 = importlib.import_module(
        "scripts.run_private_speaker_review_next_adjudication_observation"
    )
...
class ThirdAdjudicationSubmissionError(RuntimeError):
...
ThirdAdjudicationProcessingError = ThirdAdjudicationSubmissionError
...
PHASE72_RECEIPTS_ROOT: Final = phase72_host.REVIEW_NEXT_ADJUDICATION_OBSERVATION_RECEIPTS_ROOT
RECEIPTS_ROOT: Final = host.REVIEW_THIRD_ADJUDICATION_RECEIPTS_ROOT
ENV_FILE: Final = host.ENV_FILE
COMPOSE_PATH: Final = RELEASE_ROOT / "deploy/compose.yaml"
ROOT_UID: Final = 0
ROOT_GID: Final = 0
WORKER_UID: Final = host.REVIEW_THIRD_ADJUDICATION_WORKER_UID
WORKER_GID: Final = host.REVIEW_THIRD_ADJUDICATION_WORKER_GID
...
_ROOT_INTENT_KEYS = frozenset(
    {
        ...
        "third_adjudication_part_number",
        "next_request_sha256",
        "operation",
        "phase72_observation_intent_sha256",
        "phase72_observation_receipt_sha256",
...
def _directory(path: Path, *, mode: int, owner: tuple[int, int]) -> None:
    try:
        ...
        raise ThirdAdjudicationSubmissionError("adjudication evidence unavailable") from error
...
def _decode(raw: bytes) -> dict[str, object]:
    ...
    try:
        ...
        raise ThirdAdjudicationSubmissionError("adjudication evidence invalid") from error
    if not isinstance(value, dict) or _canonical(value) != raw:
        raise ThirdAdjudicationSubmissionError("adjudication evidence invalid")
    return value
...
def _repair_linked_publication(path: Path) -> None:
    ...
    try:
        ...
        raise ThirdAdjudicationSubmissionError("adjudication record unavailable") from error


def _read_request(stream: BinaryIO) -> dict[str, object]:
    raw = stream.readline(contract.REQUEST_MAX_BYTES + 1)
    if stream.read(1):
        raise ThirdAdjudicationSubmissionError("invalid adjudication request")
    try:
        return contract.parse_request(raw)
    except (TypeError, ValueError) as error:
        raise ThirdAdjudicationSubmissionError("invalid adjudication request") from error
...
def _validate_authorization(request: Mapping[str, object]) -> str:
    ...
    try:
        ...
        raise ThirdAdjudicationSubmissionError("adjudication authorization invalid") from error
    return _sha(raw)
...
def _run_directory(request: Mapping[str, object]) -> Path:
    ...
    try:
        ...
        raise ThirdAdjudicationSubmissionError("adjudication run invalid") from error
...
def _inventory(
    ...
    except ThirdAdjudicationSubmissionError:
        raise
    except Exception as error:
        raise ThirdAdjudicationSubmissionError("adjudication inventory invalid") from error


def _cost_micros(value: object) -> int:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise ThirdAdjudicationSubmissionError("adjudication cost invalid")
    try:
        decimal = Decimal(str(value))
    except (InvalidOperation, ValueError) as error:
        raise ThirdAdjudicationSubmissionError("adjudication cost invalid") from error
...
def _phase72_request(
    request: Mapping[str, object], intent: Mapping[str, object]
) -> tuple[dict[str, object], bytes, str]:
    """Load and authenticate the Phase 72 observation authorization."""
    ...
    raw = _stable(
        ...
        maximum=phase72_contract.REQUEST_MAX_BYTES,
        mode=0o600,
        owner=(ROOT_UID, ROOT_GID),
    )
    phase_request = phase72_contract.parse_request(raw)
    expected = {
        "archive_sha256": request["archive_sha256"],
        "authorization_id": authorization_id,
        "maximum_authorized_cost_microusd": maximum,
        "operation": phase72_contract.OPERATION,
        "purpose": phase72_contract.PURPOSE,
        "run_id": request["run_id"],
        "schema_version": phase72_contract.PROTOCOL_VERSION,
        "season_number": phase72_contract.SEASON_NUMBER,
    }
...
def _validate_phase72_predecessor(
    request: Mapping[str, object],
    run: Path,
    contents: Mapping[str, bytes],
    state: SpeakerReviewRunState,
) -> tuple[dict[str, object], str, str, str, int, int]:
    """Authenticate Phase 72's observation receipt and its full prefix chain."""
    try:
        _directory(PHASE72_RECEIPTS_ROOT, mode=0o700, owner=(ROOT_UID, ROOT_GID))
        intent, intent_sha = _record(PHASE72_RECEIPTS_ROOT / f"{request['run_id']}.intent.json")
        receipt, receipt_sha = _record(PHASE72_RECEIPTS_ROOT / f"{request['run_id']}.json")
        if set(intent) != phase72._BINDING_KEYS or set(receipt) != phase72._RECEIPT_KEYS:
            raise ValueError
        phase_request, phase_auth_raw, phase_auth_sha = _phase72_request(request, intent)
        validated_intent = phase72._validate_intent(
        ...
        pre_contents.pop("adjudication-part-0002-output.jsonl", None)
        pre_contents.pop("adjudication-part-0002-api-errors.jsonl", None)
        ...
        phase72._validate_final_receipt(
            receipt, intent=validated_intent, result=result, contents=clean_contents
        )
        if (
        ...
        raise ThirdAdjudicationSubmissionError("phase 72 predecessor invalid") from error
...
def _pre_submission_snapshot(
    ...
    try:
        ...
        raise ThirdAdjudicationSubmissionError("adjudication replay invalid") from error
...
def _validate_state_shape(state: SpeakerReviewRunState) -> None:
    try:
        ...
        raise ThirdAdjudicationSubmissionError("adjudication checkpoint invalid") from error
    if state.adjudication_part_count <= 2:
        raise ThirdAdjudicationSubmissionError("no adjudication part remains")
    # Phase 72 authenticated observation of part two; this boundary authorizes
    # exactly part three and never generalizes to later parts.
    if state.adjudication_completed_part_count != 2:
        raise ThirdAdjudicationSubmissionError("adjudication predecessor invalid")
    if state.status not in {
        SpeakerReviewRunStatus.ADJUDICATION_PART_COMPLETED,
        SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED,
    }:
        raise ThirdAdjudicationSubmissionError("adjudication checkpoint invalid")
...
def _binding(
    ...
    *,
    ...
    phase72_intent_sha256: str,
    phase72_receipt_sha256: str,
    estimated: int,
    contents: Mapping[str, bytes],
    state: SpeakerReviewRunState,
) -> dict[str, object]:
    ...
    if next_name not in contents:
        raise ThirdAdjudicationSubmissionError("adjudication request unavailable")
    return {
        ...
        "third_adjudication_part_number": next_part,
        "next_request_sha256": _sha(contents[next_name]),
        "operation": contract.OPERATION,
        "phase72_observation_intent_sha256": phase72_intent_sha256,
        "phase72_observation_receipt_sha256": phase72_receipt_sha256,
...
def _validate_binding(value: object, request: Mapping[str, object]) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != _ROOT_INTENT_KEYS:
        raise ThirdAdjudicationSubmissionError("adjudication intent invalid")
    if (
...
def _write_once(path: Path, value: Mapping[str, object]) -> None:
    ...
    if len(raw) > MAX_RECORD_BYTES:
        raise ThirdAdjudicationSubmissionError("adjudication receipt unavailable")
    if os.path.lexists(path):
        existing, _ = _record(path)
        if existing != dict(value):
            raise ThirdAdjudicationSubmissionError("adjudication receipt conflict")
        return
    pending = path.with_name(f".{path.name}.pending")
    descriptor = -1
    try:
        ...
        existing, _ = _record(path)
        if existing != dict(value):
            raise ThirdAdjudicationSubmissionError("adjudication receipt conflict") from None
        pending.unlink(missing_ok=True)
    except OSError as error:
        raise ThirdAdjudicationSubmissionError("adjudication receipt unavailable") from error
    finally:
        if descriptor >= 0:
            os.close(descriptor)
...
def _worker_args(
    ...
    command = [
        ...
        os.fspath(ENV_FILE),
        "--profile",
        host.REVIEW_THIRD_ADJUDICATION_COMPOSE_PROFILE,
        "-f",
        ...
        "never",
        "--name",
        host.REVIEW_THIRD_ADJUDICATION_CONTAINER_NAME,
        "--user",
    ...
    command.extend(
        (
            "--volume",
            f"{(run_parent / str(request['run_id'])).as_posix()}:{(host.REVIEW_THIRD_ADJUDICATION_RUNS_TARGET / str(request['run_id'])).as_posix()}:rw",
            host.REVIEW_THIRD_ADJUDICATION_COMPOSE_SERVICE,
        )
    )
    return command
...
def _container_identity_is_exact(
    ...
    try:
        expected = subprocess.run(
            ...
            check=False,
            shell=False,
            timeout=host.REVIEW_THIRD_ADJUDICATION_KILL_AFTER_SECONDS,
        )
        ...
        if (
            ...
            or not image
            or value.get("Name") != f"/{host.REVIEW_THIRD_ADJUDICATION_CONTAINER_NAME}"
            or config.get("Image") != image
            or config.get("User") != f"{WORKER_UID}:{WORKER_GID}"
            or config.get("WorkingDir") != host.REVIEW_THIRD_ADJUDICATION_CONTAINER_WORKDIR
            or config.get("Cmd") != list(host.REVIEW_THIRD_ADJUDICATION_CONTAINER_COMMAND)
            or labels.get("com.docker.compose.service")
            != host.REVIEW_THIRD_ADJUDICATION_COMPOSE_SERVICE
            or labels.get("com.docker.compose.oneoff") != "True"
            or labels.get("com.docker.compose.project")
            != host.REVIEW_THIRD_ADJUDICATION_COMPOSE_PROJECT
            ...
            or set(networks) != {host.REVIEW_THIRD_ADJUDICATION_NETWORK}
        ):
            return False
        ...
        expected_destination = host.REVIEW_THIRD_ADJUDICATION_RUNS_TARGET
        ...
        return (
            ...
            == {
                ...
                host.REVIEW_THIRD_ADJUDICATION_SECRET_TARGET,
                host.REVIEW_THIRD_ADJUDICATION_TMP_TARGET,
            }
        )
    except (
        OSError,
...
def _cleanup_worker(request: Mapping[str, object], runs: Path, bindings: Mapping[str, str]) -> None:
    ...
    try:
        subprocess.run(
            ["docker", "rm", "--force", host.REVIEW_THIRD_ADJUDICATION_CONTAINER_NAME],
            cwd=RELEASE_ROOT,
            env=_safe_env(),
            stdin=subprocess.DEVNULL,
...
def _terminate_worker(process: subprocess.Popen[bytes]) -> None:
    ...
    try:
        if os.name == "posix":
            ...
            process.wait(timeout=host.REVIEW_THIRD_ADJUDICATION_KILL_AFTER_SECONDS)
            return
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
            ...
            try:
                ...
                raise ThirdAdjudicationSubmissionError("adjudication worker timeout") from error
        ...
        try:
            ...
            raise ThirdAdjudicationSubmissionError("adjudication aggregate invalid") from error
    except ThirdAdjudicationSubmissionError:
        raise
    except (OSError, subprocess.SubprocessError, TimeoutError) as error:
        raise ThirdAdjudicationSubmissionError("adjudication worker failed") from error
    finally:
...
def _post_inventory(run: Path):
    ...
    if first != second:
        raise ThirdAdjudicationSubmissionError("adjudication post-inventory changed")
    return second
...
def _validate_worker_result(
    ...
    *,
    ...
    try:
        ...
        raise ThirdAdjudicationSubmissionError("adjudication worker result invalid") from error
    expected_run_status = (
    ...
    if (
        ...
        raise ThirdAdjudicationSubmissionError("adjudication worker result invalid")
    return result
...
def _state_transition(
    ...
    if set(before_payload) != set(after_payload):
        raise ThirdAdjudicationSubmissionError("adjudication post-state invalid")
    if any(
        before_payload[key] != after_payload[key]
        for key in before_payload
        if key not in ROOT_STATE_MUTABLE
    ):
        raise ThirdAdjudicationSubmissionError("adjudication immutable state changed")
    if (
    ...
    *,
    ...
    if result_status in {"submitted", "already_submitted"}:
        if added - allowed_journals:
            raise ThirdAdjudicationSubmissionError("adjudication post-inventory invalid")
        if any(
            name in after_files
            for name in (_OUTPUT_NAME.format(part=part), _API_ERRORS_NAME.format(part=part))
        ):
            raise ThirdAdjudicationSubmissionError("adjudication output invalid")
        if result_status == "submitted":
            _state_transition(before, after)
        elif before != after:
            raise ThirdAdjudicationSubmissionError("adjudication replay changed state")
    elif result_status == "reconciliation_required":
        if added or before_files != after_files or before != after:
            raise ThirdAdjudicationSubmissionError("adjudication reconciliation changed state")
    else:
        raise ThirdAdjudicationSubmissionError("adjudication worker result invalid")
...
def _validate_receipt(
    ...
    *,
    ...
    try:
        ...
        raise ThirdAdjudicationSubmissionError("adjudication receipt invalid") from error
...
def process_request(request: Mapping[str, object]) -> dict[str, object]:
    try:
        ...
        if receipt_exists and not intent_exists:
            raise ThirdAdjudicationSubmissionError("orphan adjudication receipt")

        # Phase 72's receipt binds the checkpoint before part-three
        # submission. Reconstruct that checkpoint first on replay; validating
        # Phase 72 directly against the three-ID submitted state is invalid.
        ...
        if state.status is SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED:
            if not intent_exists:
                raise ThirdAdjudicationSubmissionError("adjudication intent missing")
        ...
        if estimated != prior_estimate:
            raise ThirdAdjudicationSubmissionError("adjudication estimate changed")
        ...
        if prior_actual + prior_estimate > maximum or prior_actual + prior_estimate > configured:
            raise ThirdAdjudicationSubmissionError("adjudication cost exceeds authorization")
        ...
        if state.status is SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED:
            ...
            recomputed = _binding(
                ...
                phase72_intent_sha256=phase72_intent_sha,
                phase72_receipt_sha256=phase72_receipt_sha,
                estimated=estimated,
                contents=pre_contents,
                state=pre_state,
            )
            if intent != recomputed:
                raise ThirdAdjudicationSubmissionError("adjudication intent binding changed")
            try:
            ...
            if preparation._active_binding() != (
                ...
                raise ThirdAdjudicationSubmissionError("active runtime changed")
            if receipt_exists:
        ...
        if intent_exists:
            ...
            intent = binding
        if receipt_exists:
            raise ThirdAdjudicationSubmissionError("adjudication receipt state invalid")
        ...
        try:
            ...
            raise ThirdAdjudicationSubmissionError("adjudication post-state invalid") from error
        result = _aggregate(
            request,
            after_state,
            status="submitted",
...
def _require_root() -> None:
    if (
        ...
        raise ThirdAdjudicationSubmissionError("invalid adjudication caller")
...
def main() -> int:
    try:
        ...
        sys.stderr.write("error=speaker_review_third_adjudication_rejected\n")
        return 2
