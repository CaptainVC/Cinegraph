"""Pinned-SSH client for the first adjudication submission boundary."""
...
class FirstAdjudicationSubmissionClientError(RuntimeError):
...
def _contract() -> ModuleType:
    try:
        module = import_module(
            "scripts.private_speaker_review_first_adjudication_submission_contract"
        )
        for name in (
            "COMMAND",
            "OPERATION",
        ...
        raise FirstAdjudicationSubmissionClientError("submission contract unavailable") from error


def ssh_arguments(*, ssh: str, identity: Path, known_hosts: Path, host: str) -> list[str]:
    """Build the hardened, shell-free SSH invocation for this exact command."""

    policy = import_module("scripts.private_speaker_review_first_adjudication_host_contract")
    if policy.REVIEW_FIRST_ADJUDICATION_COMMAND != _contract().COMMAND:
        raise FirstAdjudicationSubmissionClientError("submission host policy unavailable")
...
def _run_ssh(arguments: list[str], wire: Path) -> subprocess.CompletedProcess[bytes]:
    policy = import_module("scripts.private_speaker_review_first_adjudication_host_contract")
    timeout = (
        int(policy.REVIEW_FIRST_ADJUDICATION_TIMEOUT_SECONDS)
        + int(policy.REVIEW_FIRST_ADJUDICATION_KILL_AFTER_SECONDS)
        ...
        with wire.open("rb") as source:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                ...
                try:
                    ...
                    process.kill()
                    process.wait()
                    raise FirstAdjudicationSubmissionClientError("SSH submission failed") from error
                stdout = out.result(timeout=5)
                stderr = err.result(timeout=5)
        return subprocess.CompletedProcess(arguments, code, stdout, stderr)
    except FirstAdjudicationSubmissionClientError:
        raise
...
def submit_first_adjudication(
    *,
    ...
    try:
        ...
        if ssh is None:
            raise OSError
        with tempfile.TemporaryDirectory(prefix="cinegraph-speaker-adjudication-") as name:
        ...
        raise FirstAdjudicationSubmissionClientError(
            "remote adjudication submission failed"
        ) from error
    ...
    if (
        ...
        raise FirstAdjudicationSubmissionClientError("remote adjudication submission rejected")
    try:
        ...
        raise FirstAdjudicationSubmissionClientError(
            "remote adjudication response invalid"
        ) from error
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = submit_first_adjudication(
    ...
    except (FirstAdjudicationSubmissionClientError, ValueError):
        sys.stderr.write(
            '{"error":"speaker_review_first_adjudication_rejected","status":"error"}\n'
        )
        return 2
