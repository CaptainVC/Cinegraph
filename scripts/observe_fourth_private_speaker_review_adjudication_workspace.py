"""Observe exactly adjudication part four for a private review run.

This worker performs one read-only provider observation. It never submits a
Batch, polls in a loop, submits a later adjudication part, computes the final
decision, or advances beyond the part-four completion checkpoint. The root
host supplies the digest-bound ``review-runs`` mount and exact request
...
from scripts import (
    private_speaker_review_fourth_adjudication_observation_contract as contract,  # noqa: E402
)
...
class FourthAdjudicationObservationWorkerError(RuntimeError):
...
def read_stable_openai_secret(path: Path = OPENAI_SECRET_PATH) -> str:
    ...
    try:
        ...
        raise FourthAdjudicationObservationWorkerError("observation secret unavailable") from error
    finally:
        if descriptor >= 0:
            os.close(descriptor)
    if (
        ...
        raise FourthAdjudicationObservationWorkerError("observation secret unavailable")
    try:
...
def request_from_environment(environment: Mapping[str, str] | None = None) -> dict[str, object]:
    ...
    try:
        ...
        raise FourthAdjudicationObservationWorkerError("observation request invalid") from error
...
def _parse_cost_cap(value: str) -> int:
    try:
        ...
        raise FourthAdjudicationObservationWorkerError("observation request invalid") from error
    if str(parsed) != value:
        raise FourthAdjudicationObservationWorkerError("observation request invalid")
...
def _cost_microusd(value: float) -> int:
    if not isinstance(value, (int, float)) or isinstance(value, bool) or not math.isfinite(value):
        raise FourthAdjudicationObservationWorkerError("observation cost invalid")
    try:
        micros = Decimal(str(value)) * Decimal(1_000_000)
    except (InvalidOperation, ValueError) as error:
        raise FourthAdjudicationObservationWorkerError("observation cost invalid") from error
...
def _run_directory(run_id: str, review_root: Path = PRIVATE_REVIEW_RUNS_ROOT) -> Path:
    ...
    try:
        if candidate.resolve(strict=False).parent != root:
            raise FourthAdjudicationObservationWorkerError("observation run unavailable")
    except OSError as error:
        raise FourthAdjudicationObservationWorkerError("observation run unavailable") from error
    return candidate
...
def _read_checkpoint_file(path: Path, maximum: int) -> bytes:
    ...
    try:
        ...
        raise FourthAdjudicationObservationWorkerError(
    ...
    if (
        ...
        raise FourthAdjudicationObservationWorkerError("observation checkpoint changed")
    return raw


def _expected_hash(value: str, label: str) -> str:
    if len(value) != 64 or any(character not in "0123456789abcdef" for character in value):
        raise FourthAdjudicationObservationWorkerError(f"observation {label} invalid")
    return value
...
def _validate_expected_checkpoint(
    ...
    if any(value is None for value in expected.values()):
        raise FourthAdjudicationObservationWorkerError("observation checkpoint invalid")
    if (
        ...
        or len(set(state.adjudication_batch_ids)) != contract.OBSERVED_PART_NUMBER
        or len(set(state.adjudication_input_file_ids)) != contract.OBSERVED_PART_NUMBER
    ...
    for name in sorted(names):
        ...
        if total > 256 * 1024 * 1024:
            raise FourthAdjudicationObservationWorkerError("observation checkpoint unavailable")
        contents[name] = raw
...
def _aggregate(
    ...
    *,
    ...
    try:
        ...
        raise FourthAdjudicationObservationWorkerError("observation result invalid") from error
...
def _validate_state_transition(
    ...
    if (
        ...
        raise FourthAdjudicationObservationWorkerError("observation result invalid")
...
def observe_fourth_adjudication(
    *,
    ...
    if state.status is SpeakerReviewRunStatus.ADJUDICATION_SUBMITTED:
        if state.adjudication_completed_part_count != contract.PREDECESSOR_COMPLETED_PART_COUNT:
            raise FourthAdjudicationObservationWorkerError("observation run state invalid")
    ...
    if expected_total > cap or expected_total > configuration_cap:
        raise FourthAdjudicationObservationWorkerError("observation cost exceeds authorization")
    # A completed fourth adjudication part is the only safe replay state for this
    ...
    if state.status is SpeakerReviewRunStatus.ADJUDICATION_PART_COMPLETED:
        ...
        try:
            ...
            # after this worker pins the Phase 76 request, checkpoint, and
            # part-four inventory contract above.
            returned_directory, replayed = _replay_workflow().observe_next_adjudication(
                canonical,
                verified_run_state=state,
            )
        except (AssertionError, RuntimeError) as error:
            raise FourthAdjudicationObservationWorkerError("observation replay invalid") from error
        if returned_directory != canonical or replayed != state:
            raise FourthAdjudicationObservationWorkerError("observation replay invalid")
        return _aggregate(
            state,
    ...
    try:
        ...
        raise FourthAdjudicationObservationWorkerError(
            "fourth adjudication observation failed"
        ) from error
...
def main() -> int:
    try:
        result = observe_fourth_adjudication()
        sys.stdout.buffer.write(contract.canonical_json(result))
        return 0
    except FourthAdjudicationObservationWorkerError:
        sys.stderr.write("error=speaker_review_fourth_adjudication_observation_failed\n")
        return 2
    except Exception:
        sys.stderr.write("error=speaker_review_fourth_adjudication_observation_failed\n")
        return 2
